...
export type TechnicianSession = { contexts: MeContext[]; activeContextId: string; contextVersion: string; capabilities: string[]; navigation: NavigationItem[] }
...
export async function loadTechnicianSession(api: WebApiClient, preferredContextId?: string): Promise<TechnicianSession> {
  const contextsResult = await api.request<ContextsResponse>({ path: '/me/contexts' })
  const contexts = contextsResult.data.contexts.filter((item) => item.portal === 'TECHNICIAN')
  if (contexts.length === 0) throw new Error('当前主体没有可用的 TECHNICIAN 上下文')
  if (preferredContextId && !contexts.some((item) => item.contextId === preferredContextId)) throw new Error('TECHNICIAN 上下文不属于当前主体')
  ...
  if (capabilities.data.portal !== 'TECHNICIAN' || navigation.data.portal !== 'TECHNICIAN') throw new Error('服务端返回了错误 Portal 的上下文数据')
