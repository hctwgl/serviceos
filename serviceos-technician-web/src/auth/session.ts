...
const verifierKey = 'serviceos.technician.oidc.pkceVerifier'
const stateKey = 'serviceos.technician.oidc.state'
const returnPathKey = 'serviceos.technician.oidc.returnPath'
...
export async function completeLogin(search: string) {
  ...
  sessionStorage.removeItem(verifierKey)
  sessionStorage.removeItem(stateKey)
  const returnPath = sessionStorage.getItem(returnPathKey) ?? '/technician-portal/task-feed'
  sessionStorage.removeItem(returnPathKey)
