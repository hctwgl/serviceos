...
H5 是在线契约、产品走查和有限应急的参考实现，不承诺原生级定位、后台可靠上传、杀进程恢复、
长期本地敏感数据保存或完整离线能力。M262 已在当前任务详情加入用户主动触发的一次性浏览器定位签到和
无法施工中断；浏览器定位不等于原生设备可信度。M263 增加冻结表单的基础字段在线渲染与不可变提交；输入
只保存在当前页面内存，条件表达式、远程选项和高级控件不受支持时失败关闭，不声明草稿或离线恢复。M264
增加 Evidence 槽位/安全资料摘要与 SHA-256 → Begin → 受限 PUT → Finalize；数据面 PUT 不携带 Token/Context，
STORED 不冒充扫描完成，也不提供后台/离线上传。M265 增加 TASK_SUBMISSION Snapshot 与任务完成：客户端只选
最新 VALIDATED Revision/FormSubmission UUID，规范引用、摘要和输入版本由服务器重读冻结。没有真实
FieldOperation `operationRefs` 时仍不会开放签退或伪造现场完成。
