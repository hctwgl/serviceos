...
- `serviceos-architecture/docs/implementation-status.md`：当前基线、完成和未完成边界；
...
1. 明确能力、模块、里程碑或缺陷范围；
2. 查看实施状态和直接相关事实源；
3. 检查公共 API、依赖方向及适用契约/迁移；
4. 写下本次明确不做的边界；
5. 选择 R0～R3 和验证命令；
6. 开始编码。

禁止先写实现，再寻找文档背书；也禁止因非核心细节不完整而无限停留在分析阶段。对可逆、局部、不改变业务语义的细节可作合理假设并在交付说明中记录。
