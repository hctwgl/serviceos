...
以下为 Cursor Cloud Agent 环境下的启动/运行注意事项。标准命令以 `README.md`、`serviceos-deploy/README.md`、`.github/workflows/verify.yml` 为准，本节只记录非显然要点。

### 运行时依赖（Docker）

- 本仓库的构建与运行强依赖 Docker：PostgreSQL 18 与 Keycloak 通过 `serviceos-deploy/compose.yaml` 启动，`./mvnw clean verify` 的 PostgreSQL Testcontainers 集成测试也需要容器运行时。Docker 已随快照安装。
- Docker 守护进程不会随 VM 自动启动，也不使用 systemd。每个会话开始前需手动启动并放开 socket 权限（否则以 `ubuntu` 用户运行的 Maven/Testcontainers 无法访问 socket）：

  ```bash
  sudo dockerd >/tmp/dockerd.log 2>&1 &   # 建议放在 tmux 会话中常驻
  sleep 5 && sudo chmod 666 /var/run/docker.sock
  docker info | grep -i 'storage driver'  # 预期 fuse-overlayfs
  ```

- 守护进程配置为 `fuse-overlayfs` 存储驱动并使用 `iptables-legacy`，这是 Firecracker VM 内的既定要求，不要改回 overlay2/nftables。

### 启动本地依赖与后端

```bash
docker compose -f serviceos-deploy/compose.yaml up -d postgres keycloak
./mvnw -pl serviceos-backend spring-boot:run     # API + 进程内 worker，单 JVM，端口 8080
```

- 后端就绪探针只包含 `db`，不含 IdP：`curl -sf http://127.0.0.1:8080/readyz`。
- Flyway 在首次启动时建表（幂等）。首次建表后必须执行本地 RoleGrant，否则任何写命令都会因缺少授权而失败关闭：

  ```bash
  docker compose -f serviceos-deploy/compose.yaml exec -T postgres \
    psql -U serviceos_app -d serviceos < serviceos-deploy/keycloak/grant-local-project-admin.sql
  ```

### 认证端到端（获取真实 JWT）

- `serviceos-local-cli` 是启用 PKCE 的公有客户端，且 `directAccessGrantsEnabled=false`，因此无法用密码直连换取 token，必须走 Authorization Code + PKCE 流程（可用 curl 脚本完成表单登录并交换授权码）。
- 令牌 `iss` 必须与后端配置的 `SERVICEOS_OIDC_ISSUER_URI`（默认 `http://localhost:8081/realms/serviceos`）逐字符一致。用 `localhost:8081` 而非 `127.0.0.1:8081` 发起授权，否则 issuer 校验失败返回 401。
- 导入的 `developer` 用户没有 email，会触发 Keycloak `VERIFY_PROFILE` required action 打断授权码流程。用引导管理员（`admin` / `local-admin-change-me`，realm `master`，client `admin-cli`）通过 Admin REST API 为该用户补 email 并清空 requiredActions 后即可完成登录。该调整只作用于 Keycloak 容器运行时状态，不修改仓库文件。
- token 中的 `capabilities` claim 只是声明；真正放行写命令依赖数据库中的有效 RoleGrant（见上一步）。

### hello-world 冒烟（创建项目）

获取到 token 后执行核心写命令，预期返回 `201` 且 `prj_project` 与 `rel_outbox_event` 在同一事务写入：
...
curl -s -X POST http://127.0.0.1:8080/api/v1/projects \
  ...
  -d '{"code":"HELLO-2026","clientId":"client-demo","name":"演示项目","startsOn":"2026-01-01"}'
```

### 已知基线：15 个时间戳精度集成测试失败

- `./mvnw clean verify` 当前在 master 上稳定出现 15 个失败（`Tests run: 147, Failures: 15`），与 CI 完全一致。全部为同一根因：内存中的 `Instant` 为纳秒精度（如 `...369862092Z`），PostgreSQL `timestamp` 截断为微秒（如 `...369862Z`），断言按 `equals` 比较因而不等。
- 这是与宿主机时钟纳秒分辨率相关的既有仓库/测试问题，非环境搭建缺陷，也非本次改动引入。除非任务明确要求修复该断言，否则不要为“绿灯”而放宽或跳过这些测试。
