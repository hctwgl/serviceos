...
ServiceOS 是面向新能源充电设施现场服务的可配置履约平台，采用 Java 21、Spring Boot、Spring Modulith、PostgreSQL、Flyway、jOOQ、OpenAPI、事件 Schema、Inbox/Outbox、OIDC/JWT、Capability 和 Tenant/Project Scope。
...
- Domain 不得依赖 Web、jOOQ、JDBC、消息客户端或外部 SDK；
...
> ADR-091（Accepted，2026-07-20）已将业务数据访问层一次性统一为 **jOOQ**（jOOQ 3.21，
> Spring Boot 4.1 自动装配 DSLContext），MyBatis/JdbcClient 旧实现已当场删除，无双轨与兜底。
> 决策与工程约束见 `serviceos-architecture/decisions/ADR-091-jooq-unified-data-access.md`；
> `serviceos-architecture/architecture/36-persistence-engineering-guideline.md` 的选型条款已被取代，
> 其 Repository 语义、并发条件、多租户 Scope、事务边界与测试要求由 ADR-091 §3.4 继承。

- 业务持久化（命令侧与查询侧）统一使用 jOOQ；禁止重新引入 MyBatis、MyBatis XML、MyBatis-Plus 或业务 JdbcClient/JdbcTemplate；
- jOOQ 生成物位于 `serviceos-backend/src/generated/java/`，由 `bash scripts/generate-jooq.sh` 从 Flyway 基线重新生成，禁止手改；`bash scripts/check-jooq-generated.sh` 校验生成物与迁移基线一致；
- UUID/JSONB/timestamptz 绑定统一走 `shared/infrastructure/jooq/` 的公共 Converter，禁止模块自行实现第二套；
- Domain/Application 通过 Repository 端口访问数据，不得依赖 jOOQ DSL、生成物类型、Mapper 或 JDBC；
...
| jOOQ/SQL/Flyway | 对应 PostgreSQL IT；Flyway 还需迁移验证 |
