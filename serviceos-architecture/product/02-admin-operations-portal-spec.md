...
| Page ID | 路由 | 页面 | MVP | 核心能力 |
|---|---|---|---:|---|
| `ADMIN.ANALYTICS.OPERATIONS` | `/analytics/operations` | 工单量、周期、积压和 SLA | 是 | `analytics.readOperations` |
| `ADMIN.ANALYTICS.QUALITY` | `/analytics/quality` | 审核、整改、返工和资料质量 | 是 | `analytics.readQuality` |
| `ADMIN.ANALYTICS.NETWORK` | `/analytics/networks` | 网点/师傅履约与产能 | 是 | `analytics.readNetwork` |
| `ADMIN.ANALYTICS.INTEGRATION` | `/analytics/integration` | 接入、回传、失败和恢复 | 是 | `analytics.readIntegration` |
| `ADMIN.ANALYTICS.FINANCE` | `/analytics/finance` | 正式收入、成本和毛利 | 二期 | `FORMAL_SETTLEMENT` + `analytics.readFinance` |
...
MVP 分析页只使用已发布 `MetricDefinitionVersion`，每个指标显示业务定义、分子/分母、时间口径、数据来源、排除条件、asOf 和负责人。

首批指标：新增/在途/完成/取消/积压，首次联系/预约/勘测/安装/审核周期，SLA 预警/超时，一次审核通过率、整改轮次、自动派单成功率、回传成功/UNKNOWN/最终失败和不可试算率。

- 所有图表可下钻到同口径受权列表；
- 网点排名按样本量和数据质量标记，不用不完整样本做惩罚；
- 运营角色默认只读，不因能看指标获得工单动作；
- 导出使用当前数据范围、字段策略、指标版本和增强审计；
- 对上/对下正式收入成本、毛利和账龄延后至 FORMAL_SETTLEMENT；M5 仅可展示明确标记 SHADOW 的试算汇总。
