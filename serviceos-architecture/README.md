...
当前工程基线推进至 **M143**：Admin 已证明补传复审完结、预约上门、BYD 提审外发 ACK、厂端回调，入站→激活→同单预约上门→表单/资料/驳回整改补传复审/外发/完结，以及 Visit 所需 ServiceAssignment 经 Dispatch SPI 编排注入（非 SQL 直插、非 Admin 派单 HTTP）；Evidence 已覆盖条件槽位初次解析、表单事实只追加重解析、槽位世代、
