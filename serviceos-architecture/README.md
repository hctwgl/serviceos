...
149. [ADR-022 条件事实版本与资料槽位重解析（待确认）](decisions/ADR-022-conditional-fact-version-and-evidence-reresolution.md)
150. [M53 表单条件与 EvidenceSlot 重解析实施提案](architecture/66-m53-form-condition-evidence-reresolution-proposal.md)
151. [M53 表单条件与 EvidenceSlot 重解析验收提案](testing/50-m53-form-condition-evidence-reresolution-acceptance-proposal.md)
...
当前工程基线推进至 **M52**：Evidence 已覆盖条件槽位初次解析、不可变 Item/Revision、机器校验、
Snapshot、Task 完成门禁、作废、Review/Correction 与条件决策审计。完整平台仍未完成：表单条件求值、
条件字段变化后的槽位重解析、OCR/CV、复杂工作流、SLA/通知/结算、正式基础设施和各 Portal 工程
