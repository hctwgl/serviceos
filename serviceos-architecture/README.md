...
当前版本为 `0.9.0`，包含 M0–M7 架构、研发和多 Portal 产品基线，以及 M8 首条可构建工程纵向切片：业务资产、履约内核、现场作业、自动化闭环、双向试算、迁移灰度、工程模块、事务消息、安全非功能、总部/网点/师傅应用与体验门禁、Project/幂等/审计/Outbox 参考实现。它不代表 M6 E1、所有车企、业务类型或页面已经实现。
