...
当前工程基线推进至 **M73**：Evidence 已覆盖条件槽位初次解析、表单事实只追加重解析、槽位世代、
...
独立 `readmodel` 模块现通过 Inbox 可靠消费 WorkOrder/Workflow/Stage/Task 公开事件，提供实时授权、
稳定分页且不含 PII/原始 payload 的工单核心执行时间线；跨域完整时间线和投影重建作业仍未实现。
完整平台仍未完成：计算字段/脚本、
