...
66. [M13 可观测性、健康探针与日志脱敏参考实现](architecture/27-observability-health-redaction-implementation.md)
67. [M13 可观测性、健康探针与日志脱敏验收矩阵](testing/11-m13-observability-acceptance.md)
68. [ADR-016 单一镜像、独立迁移与失败关闭发布](decisions/ADR-016-single-image-explicit-migration-fail-closed-deployment.md)
69. [M14 容器、独立迁移与 staging 发布参考实现](architecture/28-container-staging-deployment-implementation.md)
70. [M14 容器、迁移、staging 与回滚验收矩阵](testing/12-m14-container-staging-deployment-acceptance.md)
...
当前版本为 `0.15.0`，包含 M0–M7 架构、研发和多 Portal 产品基线、M8 首条可构建工程纵向切片、M9 OIDC/授权/可靠消息、M10 自动 Task/Scheduler/人工接管、M11 三段式安全文件生命周期、M12 契约兼容 CI/可重复客户端生成、M13 OTel/探针/指标看板/日志脱敏，以及 M14 单一镜像/独立迁移/staging smoke/回滚参考实现。M14 补齐了 E1-11 的仓库内参考证据，但它不代表 M6 Foundation 全部 P0、正式依赖、生产部署平台、所有车企、业务类型或页面已经实现。
