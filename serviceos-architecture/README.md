...
当前工程基线推进至 **M64**：Evidence 已覆盖条件槽位初次解析、表单事实只追加重解析、槽位世代、
...
Project Scope 授权进入工作台、工单时间线和详情查询；Project REGION 有效期关系可将 REGION RoleGrant
映射为同一工作台的精确项目集合，动态时间统一使用服务端 `asOf`。完整平台仍未完成：计算字段/脚本、
