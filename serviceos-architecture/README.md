...
当前版本为 `0.12.0`，包含 M0–M7 架构、研发和多 Portal 产品基线、M8 首条可构建工程纵向切片、M9 OIDC/授权/可靠消息、M10 自动 Task/Scheduler/人工接管，以及 M11 三段式安全文件生命周期参考实现。它不代表 M6 E1、正式 IdP/Broker/对象存储/反病毒服务、所有车企、业务类型或页面已经实现。
