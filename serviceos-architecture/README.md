...
当前版本为 `0.27.0`。在 M25 改派握手基线上，0.27.0 新增 TASK_PREPARED 持久检查点、切换前 ABORTING/ABORTED 可靠终止和故障后前向恢复。该版本仍不代表初派握手、Network 双级派单、派单策略、超时自动决策、切换后补偿、SLA、网关、并行、等待或比亚迪勘安全流程已经完整实现，也不代表所有 Proposed 文档已获得业务与技术签署。
