...
当前工程基线推进至 **M86**：Evidence 已覆盖条件槽位初次解析、表单事实只追加重解析、槽位世代、
...
激活生命周期、Task 指派/执行保护/人工接管、外部审核回执与条件 KEEP/INVALIDATE 处置公开事件，提供实时授权、稳定分页且不含 PII/原始 payload 的工单执行时间线；时间线投影已具备 checkpoint/dead letter/generation 重建、definition 登记、dead letter 幂等重放与旧/孤儿 generation 清理，以及 freshness 语义；并提供不含客户 PII 的工单工作区顶层实时组合查询（缺权 SLA/异常区块降级）。试算完整合并、队列/SavedView 与 sections 按需加载仍未实现。
完整平台仍未完成：计算字段/脚本、
