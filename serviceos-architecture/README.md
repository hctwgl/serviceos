...
当前版本为 `0.19.0`。在 M17 可靠启动基线上，0.19.0 新增 TaskCompleted 稳定领域事件、冻结流程版本解析、NodeInstance 运行时，以及同阶段唯一无条件任务推进。该版本仍不代表跨阶段、结束、网关、并行、人工任务或比亚迪勘安全流程已经完整实现，也不代表所有 Proposed 文档已获得业务与技术签署。
