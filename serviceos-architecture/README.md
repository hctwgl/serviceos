...
当前版本为 `0.28.0`。在 M26 可靠终止基线上，0.28.0 新增逐阶段 deadline、并发安全超时扫描、不可变 timeout occurrence、可靠异常事件和去重人工接管。该版本仍不代表初派握手、Network 双级派单、派单策略、超时后的自动 abort/继续/补偿、切换后补偿、SLA、网关、并行、等待或比亚迪勘安全流程已经完整实现，也不代表所有 Proposed 文档已获得业务与技术签署。
