...
当前版本为 `0.26.0`。在 M24 双侧本地状态机基线上，0.26.0 新增 Dispatch/Task 四段 Inbox 改派握手、事件发起人实时授权复核和失败后向前重试。该版本仍不代表初派握手、Network 双级派单、派单策略、自动 abort/切换后补偿、SLA、网关、并行、等待或比亚迪勘安全流程已经完整实现，也不代表所有 Proposed 文档已获得业务与技术签署。
