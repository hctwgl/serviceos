...
## 0. 接受范围（M84 / M86 / M189 / M190 / M191 / M192 / M193）
...
- **M192**：§9 Search **受控精确/前缀回退切片**（无 `search_document` 索引平台、无异步 indexer）。
  Admin 搜索经既有授权查询端口 fan-in；`search_document` 投影定义**仍为 Draft**，本切片不物化。
- **M193**：§8 `RecentResource` **个人 Admin 切片**（portal=`ADMIN`；owner=`principal_id`；
  物理表 `rdm_recent_resource`）。保存 tenant、principal、portal、resource_type、resource_id、
  page_id、非敏感 display_ref、last_visited_at。唯一键
  `(tenant_id, principal_id, portal, resource_type, resource_id)`。读取时重新鉴权并隐藏/删除
  失权项；不得保存完整用户/地址/电话/价格快照。不接受 Network/Technician Portal 最近访问。

**仍为设计草案、不得在未再接受前实现**：§3 工单工作区/列表其余投影扩展、§4～§5 队列/专项投影、
§6 ORGANIZATION 组织树共享与 Network/Technician SavedView、§7 非 Admin/白名单外偏好、
§8 Network/Technician RecentResource、§9 `search_document` 索引平台与异步 indexer、§10～§11 移动同步/指标，
以及多投影通用平台与 Portal、Admin 重建/重放 HTTP、Broker offset。
...
### 7.1 Admin 个人键（M190 Accepted）

物理唯一键：`(tenant_id, principal_id, portal, preference_key)`，portal 固定 `ADMIN`。

允许 preference_key：`theme`、`density`、`locale`、`reduceMotion`、`defaultSavedViews`、
`columnWidths`（语义与 API-06 §9.1 一致）。未知键与禁止键失败关闭，不得静默忽略。

## 8. RecentResource

### 8.1 个人 Admin 切片（M193 Accepted）

保存 tenant、principal、portal=`ADMIN`、resource_type、resource_id、page_id、last_visited_at
和非敏感 display_ref。物理表 `rdm_recent_resource`；唯一键
`(tenant_id, principal_id, portal, resource_type, resource_id)`。

本切片 resource_type：`WORK_ORDER`、`TASK`、`PROJECT`、`NETWORK`、`TECHNICIAN`。

读取时经既有授权查询端口重新鉴权；资源失权后隐藏，并可在读路径删除陈旧行。列表有上限。
touch/upsert 仅更新本人行。不得保存完整用户/地址/电话/价格快照。

Network/Technician Portal 最近访问**仍为 Draft**，不得在未再接受前实现。

## 9. Search 投影

### 9.1 受控精确/前缀回退（M192 Accepted）

在 `search_document` 索引平台不可用（本系统尚未物化该投影）时，Admin 全局搜索**必须**回退为
受控精确/前缀查询：经 workorder / network / identity 等既有授权端口 fan-in，先应用当前
tenant/scope 与 Capability，再返回最小化结果。不得绕过 ScopePredicate；不得从搜索结果直接
执行命令。本切片不新增搜索索引表或异步 indexer。

### search_document（Draft）
...
- 搜索先 tenant/scope 过滤；
- 删除/失权事件及时更新；
- 搜索索引不可用时回退受控精确查询（M192 已接受该回退路径）；
- 不能从索引直接执行命令。

`search_document` 投影与异步索引平台**仍为 Draft**，不得在未再接受前实现。
