...
| 派单 | DispatchRequest、Decision、Evaluation、ServiceAssignment、CapacityReservation | Task 责任人和 SLA |
...
保存层级（NETWORK/TECHNICIAN）、assignee、生效区间、来源 decision、状态、被替代 service assignment 和改派原因。同工单/任务/层级同一时刻最多一个 ACTIVE。
...
保存连接器/映射版本、消息类型、业务键、源对象精确版本、payload 快照、外部幂等键、失败策略和状态。

### delivery_attempt

保存 attemptNo、请求/响应摘要和引用、外部流水、开始/结束、错误分类、下次重试、连接器凭据版本。
...
delivery 以 intent + recipient + channel 唯一；attempt 只追加；receipt 保存供应商送达/阅读事实。
...
- connector + 状态 + nextRetryAt 的 delivery 积压；
