...
### calculation_eligibility_hold

保存 correctionRequest、旧事实、受影响 calculationRun、方向、状态（ACTIVE/RELEASED/CONVERTED_TO_ADJUSTMENT）、创建事务版本、释放/转换原因和时间。
...
只有 mode=AUTHORITATIVE、无 pricing override、pricingAuthorityVersion 仍有效且方向级 `FORMAL_SETTLEMENT` 已激活的 run 可通过正式 SettlementEligibility。该约束在资格评估和 StatementLine 写入时重复校验。
...
保存方向、对象、项目、周期、币种、税模式、状态、汇总、版本、formalSettlementFeatureVersion 和创建时 authorityVersion。
...
一条 line 的金额来源只能是 `charge_item_id` 或 `adjustment_id` 之一（XOR）。Adjustment 不派生第二个可结算 ChargeItem；展示用费用分类仅为非结算投影。
...
Settlement submit/lock/handoff 等副作用对象还必须保存 `fence_decision_id / authority_assignment_id / authority_version / fence_policy_version`；最终写入前版本变化则拒绝。
...
保存 tenant、project、cohort、direction（RECEIVABLE/PAYABLE）、authoritySystem、pricingAuthorityVersion、effectiveAt、状态和审批。它独立于工单写入权威：允许先让 ServiceOS 承接履约但仍只影子计价。每个适用范围/方向同一时刻最多一个 ACTIVE 计价权威。

### domain_command_authority_check

保存 commandId/type、workOrder、requestSystem、submittedAuthorityVersion、currentAuthorityVersion、mode、ALLOW/DENY、reason、事务时间和 correlationId。所有普通领域命令、后台 Task、批处理和连接器回放在写业务对象/Outbox 前执行该检查；其业务表写入和 ALLOW 记录共享事务边界。
...
保存 cohort、cutoverDecision、source/target authority、targetWatermark、增量起止水位/摘要、状态（DRAFT/ANALYZING/BLOCKED/READY/APPROVED/EXECUTING/VERIFYING/VERIFIED/FAILED/CANCELLED）、阻塞摘要、审批和执行检查点。

### rollback_side_effect_item

逐项保存 operationType、业务对象、delivery/notification/assignment/settlement 引用、authorityVersion、状态（PENDING/SENT/UNKNOWN/ACKNOWLEDGED/CANCELLED）、可逆性、处置决定和证据。

### rollback_sync_record

保存 plan、领域对象/版本、反向映射版本、目标业务键、幂等键、payload 摘要、同步 attempt/回执、源/目标 watermark 和结果。重试不得重复写旧系统。

### rollback_validation_result

保存计划、检查类型、预期/实际数量或摘要、金额/文件/关系差异、阻塞等级、证据、签署人和结果。只有必需检查通过才能 VERIFIED。
...
- 所有领域写命令在提交前校验 authorityVersion；
- SHADOW 模式 SideEffectFence 拒绝真实外部/财务副作用；
- Delivery/Notification/AssignmentActivation/Settlement 保存最终 fence 与 authority 决策引用；
- RollbackPlan 必须覆盖副作用清单、反向同步、阻塞和验证后才可切换；
