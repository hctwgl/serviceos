...
| `GET /technician/me/tasks/{taskId}/forms` | 当前责任师傅读取冻结表单 | `X-Technician-Context` | 200 |
| `POST /technician/me/tasks/{taskId}/form-submissions` | 当前责任师傅在线不可变提交 | `Idempotency-Key` | 201/200 |
...
Technician 专用提交只接受 `formVersionId` 与 `values`。tenant、actor、submittedBy 与 prefillVersion 均由
服务端上下文或既有事实决定，客户端不得提交；M263 客户端只支持基础类型，发现条件、远程选项、高级控件或
未知类型时必须失败关闭。页面内存输入不构成草稿或离线恢复能力。
...
| `GET /technician/me/tasks/{taskId}/evidence-slots` | 当前责任师傅安全槽位 | `X-Technician-Context` | 200 |
| `GET /technician/me/tasks/{taskId}/evidence-items` | 当前责任师傅安全资料摘要 | `X-Technician-Context` | 200 |
| `POST /technician/me/tasks/{taskId}/evidence-slots/{slotId}/upload-sessions` | 在线 Begin | 文件元数据、captureSource、capturedAt | 201/200 |
| `POST /technician/me/tasks/{taskId}/evidence-slots/{slotId}/upload-sessions/{id}:finalize` | 在线 Finalize | actualSha256、finalizeCommandId | 201/200 |
...
| `POST /technician/me/tasks/{taskId}/evidence-set-snapshots` | 当前责任师傅创建提交快照 | memberRevisionIds | 201 |
| `POST /technician/me/tasks/{taskId}:complete` | 当前责任师傅完成任务 | Snapshot UUID、FormSubmission UUID?、If-Match | 200 |

UploadSession 返回受限上传地址、允许分片、过期时间和最大字节数。Finalize 前服务器校验对象路径、长度和 checksum。

Technician 专用 Begin 不接受 tenant、uploader、offline、locationVerified、onBehalf 或对象键；服务端构造
在线采集边界并重新验证当前责任、Capability、Task guard、Slot 规则和 session owner。M264 H5/iOS 的数据面
PUT 只携带短期 URL 与 `requiredHeaders`，不得携带 Bearer Token、Technician Context 或客户端元数据。
Finalize 的 STORED/VALIDATING 状态不代表扫描或机器校验完成。
...
M265 Technician 专用 Snapshot 端点固定 `purpose=TASK_SUBMISSION`，客户端只能选择 Revision UUID。任务完成端点
只接受 Snapshot UUID 和可选 FormSubmission UUID；规范 URI、摘要及 `inputVersionRefs` 必须由服务端重新读取后
构造，并继续委托 Task 内核复核版本、状态、责任、冻结 FormVersion 和 Snapshot 当前资格。客户端不得提交或回显
内部 resultRef/digest。该闭环不产生 Visit `operationRefs`；没有 Accepted FieldOperation 映射时仍禁止签退。
