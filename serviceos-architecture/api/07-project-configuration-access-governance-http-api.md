...
> M64～M65 已在 Core OpenAPI 实现 `POST /api/v1/projects` 的 `regionCodes`/`networkIds` 子集：
> 省略/空数组表示不建立对应关系；非空值与项目、审计、Outbox、幂等结果同事务写入有效期关系。
> M66 / Core OpenAPI 0.37.0 已实现带 If-Match、幂等和不可变收据的关系即时整组修订/终止；owners、
> 计划生效/审批、项目列表/详情/激活/退役仍未实现；networkIds 不替代 ServiceNetwork 目录。
