...
| M259-02 | 五环境 | scheme/config 显式；Test 固定 `.invalid`，Production 必须显式注入 | xcconfig + Test built Info.plist assertions |
...
| M259-07 | Simulator 编译 | Test App 可链接 iOS Simulator SDK | Xcode shared scheme build |
...
没有 Booted Simulator 时不声明 XCTest/XCUITest 已运行。Production 优化构建、签名、真机安装、真实 OIDC 登录和上下文切换、
