...
| MCP-APP-04 | Technician iOS 独立构建 | SwiftUI 工程可在模拟器和开发真机构建 | M259 Test generic Simulator 与无签名 generic iPhoneOS arm64 构建通过；Production 优化构建和签名开发真机待补 |
...
| ID | 验收项 | 预期 | 证据 |
|---|---|---|---|
| MCP-IOS-01 | 环境配置 | Local/Dev/Test/Staging/Prod 不可误混 | M258 配置模型 + M259 五个 xcconfig、Test `.invalid` 产物断言；Production 显式注入构建待补 |
