...
| MOK-10 | REFERENCE_OEM / 第二家 | 独立 Connector + Bundle；不确定协议 `TBD_EXTERNAL_CONTRACT` | PASS（M272 SAMPLE；真实协议 BLOCKED_EXTERNAL） |
| MOK-11 | 双车企 E2E | BYD + OEM2 安装链路；重复/冲突/驳回/UNKNOWN/乱序有证据 | PASS 入站回归（M273）；提审/UNKNOWN 矩阵仍以 BYD M58～M60 为准 |
| MOK-12 | 第三家接入手册 | 手册步骤可执行且引用 SPI/配置资产 | PASS（M274） |
