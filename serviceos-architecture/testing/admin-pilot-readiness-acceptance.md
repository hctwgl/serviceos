...
明文 projectId / 源资源 / 现场与 SLA scope 字段深链（含剩余详情页 projectId）、运营异常 handlingTaskId 深链、专项队列关联资源深链（含外发/异常/入站源资源与目录/SLA 项目深链），以及专项队列与目录/SLA Accepted OpenAPI 筛选。`ADMIN-PILOT-09` 已由入站路径证明；派单为
