...
本 ADR 已于 2026-07-15 由用户明确接受。已接受以下三项：
...
后续若改变任一项，必须新增或替代 ADR，并同步迁移、契约与验收矩阵，不得在核心代码中增加双轨兼容。

## 2. 上下文

当前实现已在 `task.created@v1` 时根据锁定配置和 WorkOrder/Region/Task 权威事实完成条件资料的
初次解析，并冻结输入摘要与命中解释。系统已提供只追加的 `FormSubmission`，但表单条件仍被
失败关闭。当前数据模型还存在以下边界：
...
Task。后续必须通过明确的 condition disposition 命令决定保留或作废；命令要求
`evidence.condition-disposition` capability、精确 resolution/slot、原因与审核引用。普通重解析
消费者无权替用户作出该决定。
...
- Event Schema、OpenAPI、Flyway、追踪矩阵和实施状态在同一变更中同步。
