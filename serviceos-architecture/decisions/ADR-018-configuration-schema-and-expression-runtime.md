...
approved_by:
  - User authorization for conditional expression implementation scope
...
当前实现接受本 ADR 的**条件层最小子集**：布尔字面量、显式白名单路径、字符串等值/不等值、
`&&`、`||`、`!` 与括号。决策表、数值公式、脚本和插件层仍只是本 ADR 的后续设计方向，
不得因本 ADR 状态为 Accepted 而宣称已经实现。
