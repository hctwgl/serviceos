...
related_docs:
  - architecture/371-m358-supported-client-kinds-directed-publish.md
...
使“不可在该端履约”的师傅不会进入有效 TECHNICIAN 责任，与 M358/M359 失败关闭语义对齐。
...
| 选项 | 含义 |
|---|---|
| **A1-R（推荐）** | **仅自动 TECHNICIAN 派单池**（M332 `activateTechnicianFromFrozenDispatchPolicy` 候选）硬过滤；Manual / Network Portal 指派仍允许指定，但须返回可解释警告或同步预检（见 A3）。 |
| A1-B | 自动 + Manual + Network assign/reassign **全部**硬拒绝不兼容师傅。 |
| A1-C | 仅 Network Portal；Admin 人工豁免。 |

**推荐 A1-R**：先堵住自动池“静默指错端”的主风险；人工路径保留运营覆盖，失败关闭放在执行门禁（M357～M363）与可选预检。
...
| 选项 | 含义 |
|---|---|
| **A3-R（推荐）** | 自动池过滤后为空 → 与 M332 一致落入 **TECHNICIAN MANUAL**（可解释原因码，如 `CLIENT_KIND_TARGET_EMPTY`）；若接受 A1-B，Manual 不兼容 → **422** + 拒绝审计。 |
| A3-B | 自动池为空时跨区/放宽目标（禁止：破坏 M358 定向语义）。 |
...
| 选项 | 含义 |
|---|---|
| **A4-R（推荐）** | 取冻结 Bundle 内**本任务相关** FORM∩EVIDENCE 资产的 `supportedClientKinds`：**任一资产非空则取交集**；全 null = 不施加派单级 kind 过滤（仍受 M356 能力并集门禁约束于执行时）。师傅声明集合必须与该有效目标有非空交集才可通过。 |
...
| 选项 | 含义 |
|---|---|
| **A5-R（推荐）** | 派单过滤是**前置硬过滤**；M359 Feed/详情与 M357～M363 执行拒单仍保留（防声明变更、Bundle 重绑、人工覆盖）。不删除执行门禁。 |
...
## 5. 建议实施切片（仅在 Accepted 后 → M366）

1. A2-R：师傅声明能力持久化（Flyway + API/目录投影，最小读写）；
2. 冻结 Bundle 目标求交工具（复用 M358 侧表/Probe）；
...
或给出替代组合。接受后将 `status` 改为 `Accepted`，再开实现里程碑 M366 Draft PR。
