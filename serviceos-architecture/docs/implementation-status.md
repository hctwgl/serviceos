...
baselineCommit: 23f0300ee0c1d77ed2518644f58ce8f78273280d
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M305 ASSIGNEE_POLICY 运行时 |
| 基线提交 | `23f0300ee0c1d77ed2518644f58ce8f78273280d`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303/M304** INTEGRATION Mapping + **M305** ASSIGNEE_POLICY 运行时 | 条件积木、DISPATCH/NOTIFICATION/RULE/PRICING 运行时、出站 Mapping | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M305 |
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI（建单、更新、取消、出站、回调、Profile/Route） | 真实第二家协议/Sandbox（BLOCKED_EXTERNAL）、远端查询 SPI、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M273、M297～M302 |
...
ServiceOS 可靠纵向切片已推进到 **M305**。M267～M296 基线已稳定；**M297～M302** 已交付出站提审、
审核回调、取消/更新入站、Profile/Route 注册表（OpenAPI **1.0.40**；Flyway **118 / 120**）。

下一主线（已批准）：P2 配置资产运行时（DISPATCH → NOTIFICATION → RULE → PRICING）→ 业务低代码设计器 →
真实第二家车企。远端查询 SPI 与 Track F/G 仍为 `BLOCKED_EXTERNAL`。

```text
已接受推进顺序：
1. P2 续：DISPATCH → NOTIFICATION → RULE → PRICING 运行时；
2. P1 余量：远端状态查询 SPI；
