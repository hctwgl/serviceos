...
baselineCommit: PENDING_M75dc31f5e911182a6b91913e7c1bb87e2b6
latestMilestone: M75
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M75 工单 SLA 时间线事件合并 |
| 基线提交 | `PENDING_M75` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts` |
| 前端工程 | 尚未建立；已有 Admin、Network、Technician 产品与交互规格 |
| 数据库 | PostgreSQL + Flyway（当前版本 073 / 75） |
| 契约 | Core OpenAPI 0.46.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 应用只读投影 | 工作区、队列、时间线和投影运行时 | `PARTIAL` | 独立 readmodel 模块；核心执行、现场履约与 SLA Inbox 投影；授权时间线与稳定分页 | Evidence/Review/Delivery/异常/试算合并、工作区/队列、SavedView、搜索、重建、checkpoint、Portal | M73～M75 |
...
- V072 expand category 约束；Core OpenAPI 0.46.0 扩展 timeline 枚举；
...
ServiceOS 可靠纵向切片已推进到 **M75**。M61～M75 在授权只读与时间线投影上继续收敛现场履约与
Task ELAPSED SLA 可见性；没有实现完整 SLA/通知策略或整个现场履约平台。

```text
候选下一方向（优先从已确认文档中选择最小可靠切片）：
1. 继续把 Evidence/Review、Delivery/异常的公开事件并入时间线（优先有 WO/task 链接的子集），并建立
   可验证 checkpoint/重建作业；
