...
baselineCommit: 110fe2438f427c16e90408077cae8cd1001d0fc6
latestMilestone: M286
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M286 配置 Bundle 灰度与回滚 |
| 基线提交 | `110fe2438f427c16e90408077cae8cd1001d0fc6`（功能证据；合并入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 115 / 117） |
| 契约 | Core OpenAPI 1.0.31 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M285 + **M286** STABLE/CANARY 激活、晋级与回滚（OpenAPI 1.0.31） | 拖拽画布、百分比流量灰度、多级审批 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M286 |
...
ServiceOS 可靠纵向切片已推进到 **M286**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS、多实例、取消/重开/跳转，取消时配置化补偿任务（Flyway V109），标准模板族，配置设计器 API，Admin 设计器壳，Diff/审批门禁，以及 **Bundle 灰度/回滚通道**（Flyway V114～V115）。
