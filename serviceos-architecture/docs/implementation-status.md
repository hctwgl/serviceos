...
| 数据库 | PostgreSQL + Flyway（当前版本 146 / 148 migrations；M435 工单 `updated_at`；M420 师傅客户端种类声明事件；M419 授权拒绝 actor 索引；M414 品牌目录/省级骨架；M406 车企/行政区目录；M403 登录事件；M402 `identity.register`；M401 关注项目） |
