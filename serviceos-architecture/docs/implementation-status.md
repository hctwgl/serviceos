...
baselineCommit: 1f1053404e61f72ac684a5ea83c3948dcda595c4
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M284 Admin 配置设计器壳 |
| 基线提交 | `1f1053404e61f72ac684a5ea83c3948dcda595c4`（功能证据；合并入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器草稿与 Admin 壳 | `PARTIAL` | M282/M283 API + **M284** Admin 设计器壳（JSON+WORKFLOW 结构预览，catalog v17） | 拖拽画布、Diff/审批/灰度、Bundle UI | M16、M33、M36、M52～M53、M61、M268、M271、M281～M284 |
...
ServiceOS 可靠纵向切片已推进到 **M284**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS、多实例、取消/重开/跳转，取消时配置化补偿任务（Flyway V109），标准模板族，配置设计器 API，以及 **Admin 配置设计器壳**（page-registry-v17）。

下一主线：拖拽画布与 Diff/审批/灰度。
