...
baselineCommit: "038e15b6a18bb67882eda847ab0e48156e6ae3dd"
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M340 FORM ConditionBuilder 条件积木嵌入 |
| 基线提交 | `038e15b6a18bb67882eda847ab0e48156e6ae3dd`（功能提交后回填） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M340** 配置驱动履约主链路（含 Mapping DSL/强制/RouteHint + UPDATE/CANCEL Mapping + DISPATCH NETWORK/TECHNICIAN + Coverage/比例 + FORM ConditionBuilder） | 嵌套表达式 round-trip、AMOUNT/加权比例口径、BUSINESS 日历 SLA、结算落账、EVIDENCE 条件积木 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M340 |
...
ServiceOS 可靠纵向切片已推进到 **M340**（FORM ConditionBuilder：`formValues["…"]` +
visibility/visibleWhen/requiredWhen）。
M321～M340 均在 Draft stacked PRs 中（`#148→…→#167`；本切片 https://github.com/hctwgl/serviceos/pull/167）；M320 已合入 `master`（`32b902f8`）。
