...
baselineCommit: "32978ca9356a5195d4679962c87842216a1e41a2"
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M333 入站 Mapping 物化无适配器 fallback |
| 基线提交 | `32978ca9356a5195d4679962c87842216a1e41a2`（功能证据；合入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 125） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M333** 配置驱动履约主链路（含出站/入站仅 Mapping + DISPATCH NETWORK/TECHNICIAN） | 默认值/枚举/条件 DSL、强制全部 connector INBOUND Mapping、嵌套表达式、DISPATCH 地图/比例分配、BUSINESS 日历 SLA、结算落账、低代码深化 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M333 |
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI；**M311/M314/M316/M320** 吉利本地 + 三 OEM 并行冒烟 + **M317～M319** 远端查询/人工处置/批量重放；**M321/M322/M331/M333** 冻结 INBOUND/OUTBOUND Mapping（出站禁止 Profile payload；入站物化无适配器 fallback） | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、入站完全移除 OEM 兼容 Mapper、defaults/enum/condition DSL、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M316、M317、M318、M319、M320、M321、M322、M331、M333、M273、M297～M302 |
...
| 服务分配 | 网点分配、容量、改派 Saga、超时恢复 | `IMPLEMENTED` | ServiceAssignment、容量权威、改派、终止、对账和自动恢复；**M324/M332** 冻结 DISPATCH → ACTIVE NETWORK + TECHNICIAN | 完整策略评分、地图/比例分配、全部异常分支和 UI | M24～M28、M324、M332 |
...
ServiceOS 可靠纵向切片已推进到 **M333**（入站 Mapping 物化无适配器 fallback）。
M321～M333 均在 Draft stacked PRs 中（`#148→…→#160`；本切片 https://github.com/hctwgl/serviceos/pull/160）；M320 已合入 `master`（`32b902f8`）。
...
2. 强制全部 connector 配置 INBOUND Mapping / 删除 OEM 入站 Java Mapper；
3. DISPATCH：地图 scope / 比例分配；
4. 低代码高级能力仅在对应运行时已接入业务主链路后深化；
5. 吉利真实联调材料到位后提升为最高优先级；否则保持 `BLOCKED_EXTERNAL`。
