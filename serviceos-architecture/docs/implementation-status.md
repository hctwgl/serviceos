...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M368 Network Portal on-behalf NETWORK_WEB 能力门禁（ADR-089） |
| 基线提交 | `PENDING_MERGE`（合并后回填；依赖 M367） |
...
下一阶段（**AWAITING_OWNER_ACCEPT**）：负责人已选型 **D = BUSINESS 日历 SLA**，
但 ARCH-12 仍为 Proposed、M1-04 未填；须先接受
[ADR-090](../decisions/ADR-090-business-calendar-sla-slice.md) 切片边界后才能开 M369。

仍并行阻塞（不可发明推进）：

1. iOS 条件执行器全量硬阻断（本 Linux 环境多为 BLOCKED_EXTERNAL）；
2. 吉利联调；AMOUNT/加权（待业务确认口径）；
3. 其他已登记未实现项（如 CLIENT `reviewTaskId`）须负责人点名。

选型/接受入口：[自主 Agent 交接](autonomous-agent-handoff.md)。
