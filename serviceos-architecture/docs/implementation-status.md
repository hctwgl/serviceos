...
baselineCommit: 16924cadcb14383ff4661f4b1e0cffa105d9bf05
latestMilestone: M314
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M314 吉利取消/更新本地入站 |
| 基线提交 | `16924cadcb14383ff4661f4b1e0cffa105d9bf05`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI（建单、更新、取消、出站、回调、Profile/Route）；**M311/M314** 吉利 AES 本地建单/更新/取消 | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、Sandbox/签名、其余 7.x、远端查询 SPI、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M273、M297～M302 |
...
下一主线（已批准）：P3 余量属性面板；P4 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）→
