...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M72 Task 执行 Attempt 历史查询 |
| 基线提交 | `03601f9` |
...
| 契约 | Core OpenAPI 0.43.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 人工任务与执行历史 | claim/start/complete、责任、执行保护与授权任务读取 | `IMPLEMENTED` | 人工命令、候选领取、唯一责任、release/reclaim、执行保护；表单/资料完成门禁；授权工单摘要、独立 Task 队列/详情、现有命令 allowed-actions 及自动 Task Attempt 历史 | block/retry/cancel 等其他动作、HUMAN 命令时间线、Workflow Node 历史和 Review 完成条件 | M20～M23、M35、M41、M43、M69～M72 |
...
明确未实现：block/resolve-block、retry、cancel、manual-complete、完成条件预演、Workflow Node 与 HUMAN
命令历史、SLA 聚合和 Portal；自动 Task Attempt 历史已由 M72 补齐。allowed-actions 不是授权凭证，
写命令仍失败关闭。
...
明确未实现：HUMAN 命令时间线、Workflow Node 历史、跨模块工单统一时间线、Attempt retry/manual-complete
写命令、错误正文授权读取、SLA 聚合和 Portal。

## 5. 下一实施方向

ServiceOS 可靠纵向切片已推进到 **M72**。M61～M72 只实现显式 Task ELAPSED 时钟、其安全授权只读投影、
Project REGION/NETWORK 关系即时整组修订、授权项目目录/历史，以及工单目录/详情/Stage/Task 执行骨架，
并补齐独立 Task 授权队列、详情、现有人工命令 allowed-actions 与自动 Task Attempt 历史；没有猜测项目默认时长、日历、暂停、
