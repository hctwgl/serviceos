...
| 前端工程 | `serviceos-admin-web`（Vue+TS+Vite）已纳入 CI 构建，具备开发态 Keycloak PKCE，以及真实只读和 Task claim/release 局部写链路的 PR 阻断 E2E；Network/Technician 尚未建立 |
...
| Admin Portal | 总部运营后台 | `PARTIAL` | M101～M134：队列/任务/SLA/异常/外发/工单/项目目录、工作区、allowed-actions 命令面板；CI 阻断构建；开发态 Keycloak PKCE；真实只读与 Task claim/release 局部写链路的 PR 阻断 E2E | 设计系统、SavedView、正式企业 OIDC/BFF、完整履约写链路 E2E | M7 设计、M101～M134、Admin 试点基线 |
