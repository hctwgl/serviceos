...
baselineCommit: d344de2c113dc0321150ac17a7f85ecac3b852db
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M300 入站取消工单 SPI |
| 基线提交 | `d344de2c113dc0321150ac17a7f85ecac3b852db`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE | `PARTIAL` | BYD 入站建单/取消、提审、回调；**M267/M297～M300** 通用 SPI（建单、出站、回调、取消、Profile 注册表） | 真实第二家协议/Sandbox（BLOCKED_EXTERNAL）、Update/远端查询 SPI、Route 服务注册表化、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M273、M297～M300 |
...
ServiceOS 可靠纵向切片已推进到 **M300**。M267～M296 基线已稳定；**M297** 出站提审 SPI、
