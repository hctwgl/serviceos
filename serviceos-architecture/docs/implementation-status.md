...
baselineCommit: 163633f21b5e6f5e7ac20079b05c18b66a5b0d5d
latestMilestone: M250
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M250 Web auth/context/error/trace 共享基础 |
| 基线提交 | `163633f21b5e6f5e7ac20079b05c18b66a5b0d5d` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts`、`@serviceos/web-core`；由同一 Core OpenAPI 生成并经独立消费者门禁验证的 `@serviceos/core-client` 与 `ServiceOSCoreClient` |
...
|---|---|---|---|---|---|
| 工程基础 | 构建、测试、契约、可观测性、容器发布 | `IMPLEMENTED` | Maven、PostgreSQL IT、契约门禁、Trace/指标、单镜像迁移和回滚演练；同源 TypeScript/Swift Client；单源 Design Token；无角色 Web auth/context/error/trace Core | 独立客户端实际导入、生产 OIDC/BFF、iOS/Xcode/真机、正式 K8s、多故障域、PITR、制品发布/SBOM/签名、正式 Secret Manager | M8～M14、M247～M250 |
...
ServiceOS 可靠纵向切片已推进到 **M250**。身份治理 M183～M188、Admin 个人/共享 SavedView、Admin UI
...
无角色 Web auth/context/error/trace 共享包。独立页壳剩余主要是
