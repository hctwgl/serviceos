...
baselineCommit: fb348de036020c2d3d4e534e1d1faaaf42856c90
latestMilestone: M182
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M182 Admin 外发执行任务与快照成员深链 |
| 基线提交 | `fb348de036020c2d3d4e534e1d1faaaf42856c90` |
...
## 2.1 已接受的下一实施序列：身份与组织治理（编号冲突待裁决）

`master` 已接受一份身份与组织治理交付计划，原文将里程碑号记为 M135～M140。
但本分支工程基线已推进至 **M182**，且 **M135～M182 已被 Admin Pilot 写链路/深链切片占用并标记 Implemented**。

因此：

1. 工程基线仍以本节上方表格与 frontmatter 的 `latestMilestone: M182` 为准；
2. 下列身份治理目标本身保持 `ACCEPTED`，但其 **M135～M140 编号与已实现 Admin Pilot 里程碑冲突**；
3. 在项目负责人完成程序级重编号（或明确覆盖策略）前，Agent **不得**按下列旧编号实施身份治理，也不得把下列条目误读为“尚未使用的下一号段”。

| 计划内目标（原编号，冲突） | 状态 | 目标 |
|---|---|---|
| 原 M135 | `ACCEPTED` | 统一 Principal、IdentityLink、PersonProfile、Persona 与主体生命周期 |
| 原 M136 | `ACCEPTED` | 企业 Organization/OrgUnit/closure、人员任职和 LOCAL/外部权威同步 |
| 原 M137 | `ACCEPTED` | 合作组织、ServiceNetwork 人员、TechnicianProfile、网点关系与资质 |
| 原 M138 | `ACCEPTED` | Role/Capability/RoleGrant 申请审批撤销、Delegation、职责分离和授权解释 |
| 原 M139 | `ACCEPTED` | Admin 统一用户中心与真实 OIDC 治理 E2E |
| 原 M140 | `ACCEPTED` | `/me` contexts/capabilities/navigation、多 Persona 与三 Portal 上下文 |
...
| 统一主体目录 | Principal、IdentityLink、PersonProfile、Persona 与生命周期 | `ACCEPTED` | 现有 JWT principalId 和后端身份上下文可复用 | 主体目录代码、迁移、契约、目录 API、停用失权和安全测试（原计划号 M135，与已实现 Admin Pilot 冲突待重编号） | 身份治理正式路线 |
| 企业组织目录 | Organization、OrgUnit、closure、任职与主数据同步 | `ACCEPTED` | 已有概念模型和授权范围需求 | 组织权威、有效期历史、同步收据、离职联动和治理 API/UI（原计划号 M136，编号待裁决） | 身份治理正式路线 |
| 网点人员与师傅身份 | NetworkMembership、TechnicianProfile、网点关系与资质 | `ACCEPTED` | 已有 ServiceAssignment/TaskAssignment 和产品规格 | 目录运行时、身份绑定、多网点关系、资质和停用影响（原计划号 M137，编号待裁决） | 身份治理正式路线 |
| 角色与授权治理 | Role/Capability/RoleGrant 管理、审批、撤销与 Delegation | `ACCEPTED` | 实时 RoleGrant 授权运行时已实现 | 治理命令、职责分离、授权解释、历史和 Admin 操作面（原计划号 M138，编号待裁决） | 身份治理正式路线 |
| 统一用户中心 | Admin 用户、组织、网点人员、师傅、角色和授权治理 | `ACCEPTED` | Admin Web 与真实 Keycloak PKCE 基线可复用 | 用户中心页面、目录选择器、影响展示、真实 OIDC 治理 E2E（原计划号 M139，编号待裁决） | 身份治理正式路线 |
| Portal 上下文与导航 | `/me`、contexts、capabilities、navigation 与多 Persona | `ACCEPTED` | pageId/capability/独立 Portal 规格已存在 | 服务端上下文、Page Registry、缓存失权和三 Portal 接入（原计划号 M140，编号待裁决） | 身份治理正式路线 |
...
| Admin Portal | 总部运营后台 | `PARTIAL` | M101～M182：队列/任务/SLA/异常/入站/外发/工单/项目目录、工作区、allowed-actions；CI 阻断构建；开发态 Keycloak PKCE；真实只读与写链路 PR 阻断 E2E（含 ADMIN-PILOT-09、工作区各区块详情或 Task 旁路、预约/表单/资料/上门/联系详情页、核心时间线与最近活动资源深链、外部审核回执详情、审核/整改交叉深链、异常摘要→异常队列 query 水合与 handlingTaskId 深链、工作区审核/整改关联资源深链、Task 面板资源详情深链、Canonical Message 独立详情页、专项队列与目录页 query 水合及关联资源深链（含外发/异常/入站源资源、目录/SLA 项目与 Correction/Review 剩余 Accepted 字段、QueueTable 行内单元格深链、外发 executionTaskId / 快照成员深链）、外发关联资源与回执入站交叉深链、详情页明文 projectId（含剩余详情页） / 源资源 / 现场与 SLA scope 字段深链、项目与 SLA 任务交叉深链、专项队列与目录/SLA Accepted OpenAPI 筛选、入站 Envelope 授权队列） | 设计系统、SavedView、正式企业 OIDC/BFF、生产对象存储/专业扫描、评分/硬过滤派单与 ServiceNetwork 生命周期 | M7 设计、M101～M182、Admin 试点基线 |
...
- 外发详情 `executionTaskId` 类型对齐与深链；
- 资料快照 `members[].evidenceItemId` → EvidenceItem；
- Playwright `ADMIN-PILOT-08EM`。

明确未实现：FieldOperation、SavedView、企业 OIDC/BFF。

### M181：Admin QueueTable 行内单元格深链

已实现：

- `QueueTable` 可选 `linkColumns`；默认明文；
- Review/Correction/Outbound/Exception/Inbound/SLA/目录页接入；
- Playwright `ADMIN-PILOT-08QL`。

明确未实现：移除关联资源条、SavedView、FieldOperation、企业 OIDC/BFF。
...
- 前端队列类型对齐 OpenAPI；不新增 OpenAPI；
- Playwright `ADMIN-PILOT-08QR`。

明确未实现（QueueTable 行内链接已由 M181 承接）：SavedView、FieldOperation、企业 OIDC/BFF。
...
- 工单/任务目录与 SLA 工作台 → `ADMIN.PROJECT.DETAIL`；
- 不改 QueueTable 单元格渲染；不新增 OpenAPI；
- Playwright `ADMIN-PILOT-08DP`。

明确未实现：QueueTable 行内链接、SavedView、FieldOperation、企业 OIDC/BFF。

### M177：Admin 外发/异常/入站队列源资源深链

已实现：

- 外发/异常/入站队列关联资源深链；前端类型补齐 OpenAPI 字段；
- 不改 QueueTable 单元格渲染；不新增 OpenAPI；
- Playwright `ADMIN-PILOT-08QO`。

明确未实现：QueueTable 行内链接、多态 sourceId、SavedView、FieldOperation。

### M176：Admin 专项队列关联资源深链

已实现：

- 整改队列 → 源审核 / 整改 Task；审核队列 → project / task；
- 不改 QueueTable 单元格渲染；不新增 OpenAPI；
- Playwright `ADMIN-PILOT-08QC`。
...
- 异常详情 `handlingTaskId` → `ADMIN.TASK.DETAIL`；
- Pilot 种子 OPEN 异常 + HUMAN 接管 Task；不新增 OpenAPI；
- Playwright `ADMIN-PILOT-08HT`。

明确未实现：多态 sourceId 深链、FieldOperation、SavedView、企业 OIDC/BFF。
...
明确未实现：ReviewRoute/FieldOperation 详情、SavedView、企业 OIDC/BFF、真实 sandbox。

### M172：Admin 详情页明文 projectId 深链

已实现：

- 审核 / 异常 / Canonical / 外发详情 → `ADMIN.PROJECT.DETAIL`；
- 可空 projectId 条件渲染；不新增 OpenAPI；
- Playwright `ADMIN-PILOT-08PJ`。

明确未实现：FieldOperation/ReviewRoute 详情、SavedView、企业 OIDC/BFF、真实 sandbox。

### M171：Admin 外发关联资源与回执入站交叉深链

已实现：

- 外发详情 / 工作区 INTEGRATION → 源任务、源快照、源/CLIENT 审核；
- 外部审核回执 → 入站 Envelope；
- Playwright `ADMIN-PILOT-08OX`。
...
- 工单 / 任务 / SLA / 项目目录从 `route.query` 水合 Accepted 筛选；
- 复用 `firstRouteQuery`；侧栏直达默认值不变；
- Playwright `ADMIN-PILOT-08DH`。
...
- Review / Correction / Inbound / Outbound 队列从 `route.query` 水合 Accepted 筛选；
- 复用 `firstRouteQuery`；侧栏直达默认值不变；
- Playwright `ADMIN-PILOT-08QH`。
...
- 复用已 Implemented `GET /canonical-messages/{id}`（`integration.readInbound`）；
- Admin `/integration/canonical/{id}`；Envelope / INTEGRATION / 外部回执深链；
- Playwright `ADMIN-PILOT-08CM`。

明确未实现：FieldOperation 详情、SavedView、原文下载、企业 OIDC/BFF、真实 sandbox。
...
ServiceOS 可靠纵向切片已推进到 **M182**。Admin Pilot 已覆盖工作区区块详情/旁路、预约/表单/资料/上门/联系详情、
核心时间线与最近活动资源深链、外部审核回执详情、审核/整改交叉深链、异常摘要→异常队列 query 水合与 handlingTaskId 深链、工作区审核/整改关联资源深链、Task 面板资源详情深链、Canonical Message 独立详情页、专项队列与目录页 query 水合及关联资源深链（含外发/异常/入站源资源、目录/SLA 项目与 Correction/Review 剩余 Accepted 字段、QueueTable 行内单元格深链、外发 executionTaskId / 快照成员深链）、外发关联资源与回执入站交叉深链、详情页明文 projectId（含剩余详情页） / 源资源 / 现场与 SLA scope 字段深链、项目与 SLA 任务交叉深链、专项队列（含入站 Envelope）与目录筛选、`ADMIN-PILOT-09`；
...
8. FieldOperation 详情（API-03 仍为 Proposed，不得猜测读契约）；
9. 正式企业门户设计系统与 SavedView（需再接受 API-06）；其余候选项仍依赖 Proposed 文档接受。
```
