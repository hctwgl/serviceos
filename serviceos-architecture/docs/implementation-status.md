...
baselineCommit: 40386c7dc31f5e911182a6b91913e7c1bb87e2b6
latestMilestone: M74
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M74 工单现场履约时间线事件合并 |
| 基线提交 | `40386c7` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts` |
| 前端工程 | 尚未建立；已有 Admin、Network、Technician 产品与交互规格 |
| 数据库 | PostgreSQL + Flyway（当前版本 072 / 74） |
| 契约 | Core OpenAPI 0.45.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 应用只读投影 | 工作区、队列、时间线和投影运行时 | `PARTIAL` | 独立 readmodel 模块；核心执行与 Appointment/Visit/ContactAttempt Inbox 投影；授权时间线与稳定分页 | Evidence/Review/Delivery/SLA/异常/试算合并、工作区/队列、SavedView、搜索、重建、checkpoint、Portal | M73～M74 |
...
ServiceOS 可靠纵向切片已推进到 **M74**。M61～M74 只实现显式 Task ELAPSED 时钟、其安全授权只读投影、
Project REGION/NETWORK 关系即时整组修订、授权项目目录/历史，以及工单目录/详情/Stage/Task 执行骨架，
并补齐独立 Task 授权队列、详情、现有人工命令 allowed-actions、自动 Task Attempt 历史、核心工单执行
时间线与现场履约（联系/预约/Visit）事件合并；没有猜测项目默认时长、日历、暂停、
升级策略、默认负责人或尚未实现的动作，也没有实现完整 SLA/通知或整个现场履约平台。

```text
候选下一方向（优先从已确认文档中选择最小可靠切片）：
1. 继续把 Evidence/Review、Delivery/SLA/异常的公开事件并入时间线，并建立可验证 checkpoint/重建作业；
