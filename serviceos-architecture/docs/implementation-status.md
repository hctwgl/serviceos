...
baselineCommit: "394095345934965baa7c94fbbb876194cad08c16"
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M338 DISPATCH 签约比例缺口评分 |
| 基线提交 | `394095345934965baa7c94fbbb876194cad08c16`（功能证据；合入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M338** 配置驱动履约主链路（含 Mapping DSL/强制/RouteHint + DISPATCH NETWORK/TECHNICIAN + Coverage 地图 + 比例缺口） | Update/Cancel Mapping 强制、嵌套表达式、AMOUNT/加权比例口径、BUSINESS 日历 SLA、结算落账、低代码深化 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M338 |
...
ServiceOS 可靠纵向切片已推进到 **M338**（DISPATCH 签约比例缺口 ORDER_COUNT）。
M321～M338 均在 Draft stacked PRs 中（`#148→…→#165`；本切片 https://github.com/hctwgl/serviceos/pull/165）；M320 已合入 `master`（`32b902f8`）。

当前契约/迁移：OpenAPI **1.0.43**；Flyway **127**（`dsp_network_allocation_target`）。

下一阶段继续 **Configuration-Driven Fulfillment Runtime**：

1. Update/Cancel Mapping 强制与 Mapper 拆除；
