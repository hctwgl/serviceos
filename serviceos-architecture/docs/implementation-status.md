...
baselineCommit: f82926dbb8908b9a48db051bbbc93fbf216973c0
latestMilestone: M301
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M301 ExternalReviewRoute Profile 解析 |
| 基线提交 | `f82926dbb8908b9a48db051bbbc93fbf216973c0`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE | `PARTIAL` | BYD 入站建单/取消、提审、回调；**M267/M297～M301** 通用 SPI（建单、出站、回调、取消、Profile/Route 注册表） | 真实第二家协议/Sandbox（BLOCKED_EXTERNAL）、Update/远端查询 SPI、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M273、M297～M301 |
...
ServiceOS 可靠纵向切片已推进到 **M301**。M267～M296 基线已稳定；**M297** 出站提审 SPI、
