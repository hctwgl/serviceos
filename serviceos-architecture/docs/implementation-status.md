...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M91 工单工作区集成区块 |
| 基线提交 | `PENDING_M91` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts` |
| 前端工程 | 尚未建立；已有 Admin、Network、Technician 产品与交互规格 |
| 数据库 | PostgreSQL + Flyway（当前版本 079 / 81） |
| 契约 | Core OpenAPI 0.61.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 应用只读投影 | 工作区、队列、时间线和投影运行时 | `PARTIAL` | 独立 readmodel 模块；核心执行、现场履约、SLA、资料/审核/整改（含外部回执与条件 KEEP/INVALIDATE 处置）、外发交付全链路、异常确认/闭环、ServiceAssignment 与 Task 指派/Guard/人工接管 Inbox 投影；授权时间线与稳定分页；时间线 checkpoint/dead letter/generation 重建与 FRESH/LAGGING/UNKNOWN/REBUILDING freshness；definition 登记、dead letter 幂等重放与旧/孤儿 generation 清理；工单工作区顶层实时组合与 TASKS/TIMELINE_AUDIT/APPOINTMENTS_VISITS/FORMS_EVIDENCE/REVIEWS_CORRECTIONS/INTEGRATION 按需区块（无 PII/GPS/地址/note/表单 definition/资料 JSON/审核整改及集成敏感字段；缺权次级区块降级） | 试算合并、revision/slots 技术噪声、FACTS_CALCULATIONS、队列、SavedView、搜索、多投影平台、Broker offset、Portal、Admin 重建/重放 HTTP | M73～M91 |
...
ServiceOS 可靠纵向切片已推进到 **M91**。M61～M91 在授权只读、时间线投影运行时与工作区组合上继续收敛；
