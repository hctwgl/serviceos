...
baselineCommit: 7e1f11571c8e45a2b481bf07f3ab22d72505c8d0
latestMilestone: M295
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M295 剩余资产设计器 |
| 基线提交 | `7e1f11571c8e45a2b481bf07f3ab22d72505c8d0`（功能证据；合并入 `master` 后改为合并提交） |
...
| 契约 | Core OpenAPI 1.0.38 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M294 + **M295** NOTIFICATION/ASSIGNEE/INTEGRATION/PRICING 设计器（OpenAPI 1.0.38）；十大资产类型设计器齐备 | 历史回放、运行时执行引擎、指标驱动晋级 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M295 |
...
| Admin Portal | 总部运营后台 | `PARTIAL` | **M284/M287/M289/M291/M292/M294/M295** 配置设计器壳、拖拽画布、建边/条件、依赖报告、干跑轨迹、全资产类型；M101～M182：队列/任务/SLA/异常/入站/外发/工单/项目目录、工作区、allowed-actions；CI 阻断构建；开发态 Keycloak PKCE；真实只读与写链路 PR 阻断 E2E；M187～M188 用户中心与 `/me` 导航；**M189 个人 SavedView**；**M190 UI Preferences**；**M191 共享 SavedView**；**M192 受控全局搜索**；**M193 最近访问** | 历史回放 UI、设计系统、正式企业 OIDC/BFF | M7 设计、M101～M182、M187～M193、M284、M287、M289、M291、M292、M294、M295、Admin 试点基线 |
...
ServiceOS 可靠纵向切片已推进到 **M295**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS、多实例、取消/重开/跳转，取消时配置化补偿任务（Flyway V109），标准模板族，**十大配置资产类型设计器**，Admin 设计器壳，Diff/审批门禁，Bundle 灰度/回滚/停用，Workflow 拖拽画布，百分比流量灰度，画布建边/条件编辑，多槽位 CANARY 与满量自动晋级（Flyway V117），WORKFLOW 配置依赖分析，干跑模拟，通道停用失败关闭（OpenAPI 1.0.38）。
