...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M184 企业组织与任职目录 |
| 基线提交 | `2cc6fe92623061babdab824fab727d3e08fd8a00` |
...
| 数据库 | PostgreSQL + Flyway（当前版本 087 / 89） |
| 契约 | Core OpenAPI 0.77.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
M183～M184 已完成统一主体目录与企业组织任职；M185～M188 保持 `ACCEPTED`，只有对应工程证据成立后才能推进状态。
...
ServiceOS 可靠纵向切片已推进到 **M184**。统一主体目录与企业组织任职目录已经建立；没有实现
M185～M188 的网点/师傅、授权治理、Admin 用户中心和 Portal 上下文，也没有实现正式 HR Connector、
ORGANIZATION DataScope、身份缓存或跨服务身份事件。

```text
候选下一方向（优先从已确认文档中选择最小可靠切片；勿发明契约）：
1. 按已接受 M185 建立合作组织、ServiceNetwork 人员和 TechnicianProfile；状态语义仍须以已接受事实源为准；
2. 按 M186～M188 依次推进授权治理、Admin 用户中心和 Portal context/navigation；
...
7. FieldOperation 详情（API-03 仍为 Proposed，不得猜测读契约）；
8. 正式企业门户设计系统与 SavedView（需再接受 API-06）；其余候选项仍依赖 Proposed 文档接受。
```
