...
baselineCommit: e5e2927746754ce76bfb1b27f6ae82fa7251a4ae
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M324 DISPATCH 自动接入 ServiceAssignment |
| 基线提交 | `e5e2927746754ce76bfb1b27f6ae82fa7251a4ae`（功能证据；合入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 122） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321** 入站 Mapping 物化 + **M322** 出站 Mapping 生成提审 Payload + **M323** ASSIGNEE_POLICY → TaskAssignment CANDIDATE + **M324** DISPATCH → ServiceAssignment ACTIVE NETWORK | 全量字段仅 Mapping、删除 Profile 硬编码、默认值/枚举/条件 DSL、RULE/NOTIFICATION/PRICING/Admin 自动接入、嵌套表达式、DISPATCH 地图、BUSINESS 日历 SLA、结算落账 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321、M322、M323、M324 |
...
ServiceOS 可靠纵向切片已推进到 **M324**（DISPATCH 自动接入 ServiceAssignment）。
M321～M324 均在 Draft stacked PRs 中；M320 已合入 `master`（`32b902f8`）。

当前契约/迁移：OpenAPI **1.0.43**；Flyway **122**。

下一阶段继续 **Configuration-Driven Fulfillment Runtime**：

1. **RULE** 接入 Evidence/Task/Review 等真实业务门禁；
2. **NOTIFICATION** 可靠投递闭环（Outbox/Attempt/ACK）；
3. **PRICING** 履约事实与 CalculationSnapshot；
4. **UNKNOWN / Replay Admin 工作台**；
5. 吉利真实联调材料到位后提升为最高优先级；否则保持 `BLOCKED_EXTERNAL`；
6. 低代码高级能力仅在对应运行时已接入业务主链路后深化。
