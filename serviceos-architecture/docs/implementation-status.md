...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M366 派单级 supportedClientKinds 过滤（ADR-088 A1-R～A5-R） |
| 基线提交 | `PENDING_MERGE`（合并后回填；依赖 M365） |
...
| 网点人员与师傅身份 | NetworkMembership、TechnicianProfile、网点关系与资质 | `IMPLEMENTED` | 独立 `network` 模块；PartnerOrganization/ServiceNetwork；成员邀请；TechnicianProfile/多网点关系；资质只追加审核；可接单查询；清退/停用 clearance 与 ACTIVE 派单影响摘要；Admin 网点/师傅页（M187）；Network Portal 师傅只读列表（M194）；Network Portal 师傅关系绑定/终止与资质 PENDING 提交（M204）；Network Portal 本网点资质只读列表（M205）；Network Portal 师傅关系只读列表含 version（M206）；**M366** 师傅 `supportedClientKinds` 声明（V134 + declare API） | Coverage/Capability 地理硬过滤、离线工作包回收、自动改派、Portal decide/FileObject、Manual assign kind 硬拒绝 | M185、M187、M194、M204～M206、M366 |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M350** 配置驱动履约主链路（含 Mapping DSL Admin UI + DISPATCH 编辑器 + FORM/EVIDENCE ConditionBuilder + Technician 条件/规则执行器）；**M356～M363** 客户端能力发布门禁、运行时拒单、定向发布、Feed/详情头拒单、整改路径对齐、整改列表预检与领取/启动硬拒；**M366** Bundle 定向目标派单求交 | AMOUNT/加权（待确认）、BUSINESS 日历 SLA、editableWhen/defaultExpression、iOS 条件全量硬阻断 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M350、M356～M363、M366 |
...
ServiceOS 可靠纵向切片已推进到 **M366**（派单级 `supportedClientKinds` 过滤 / ADR-088 A1-R～A5-R；承接 M365）。
OpenAPI **1.0.58**；Flyway **134**。
Technician 能力门禁（M356～M363）、审核 handling Task（M364）、工作流门闸（M365）与派单 kind 过滤（M366）已闭环。

下一阶段（均需负责人确认后才能推进，不得发明规则）：

1. Manual / Network assign `supportedClientKinds` 硬拒绝（ADR-088 A1-B，另案）；
