...
baselineCommit: b2f2fa641a7ee03418931609a8ca756712dd089c
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M318 UNKNOWN Delivery 人工确认/放弃 |
| 基线提交 | `b2f2fa641a7ee03418931609a8ca756712dd089c`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 119 / 121） |
| 契约 | Core OpenAPI 1.0.42 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、`workorder.external-details-updated@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI（建单、更新、取消、出站、回调、Profile/Route）；**M311/M314/M316** 吉利本地生命周期 + **M317/M318** 远端查询 + 人工确认/放弃 | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、Sandbox/签名、其余 7.x、批量重放审批、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M316、M317、M318、M273、M297～M302 |
...
| 运营异常 | 异常工作台基础 | `PARTIAL` | 异常记录和恢复入口；M58 将外发 UNKNOWN 与 Task 最终人工事件汇入 OperationalException + HUMAN Task；M59 提供高风险人工重发事实；M60 在严格 ACK 后幂等闭环对应异常并处理事件乱序；列表/详情/确认已硬化为实时项目范围 | 其他异常类型自动闭环、批量重放、完整通知、运营中心前端和跨域异常目录 | M29、M58～M60、M100 |
...
审核回调、取消/更新入站、Profile/Route 注册表（OpenAPI **1.0.42**；Flyway **119 / 121**）。

下一主线（已批准）：吉利 Sandbox/OpenAPI 签名真实联调（BLOCKED_EXTERNAL）；批量 ReplayRequest →
真实第二家车企。远端查询 SPI 与 Track F/G 仍为 `BLOCKED_EXTERNAL`。

```text
已接受推进顺序：
1. P2 续：NOTIFICATION → RULE → PRICING 运行时；
2. P1 余量：批量 ReplayRequest 审批入口；
