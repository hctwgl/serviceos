...
baselineCommit: 4271cd9a3b4442b8976695248e2f0f63672f140e
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M317 远端状态查询 Connector SPI |
| 基线提交 | `4271cd9a3b4442b8976695248e2f0f63672f140e`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 契约 | Core OpenAPI 1.0.41 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、`workorder.external-details-updated@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI（建单、更新、取消、出站、回调、Profile/Route）；**M311/M314/M316** 吉利本地生命周期 + **M317** 远端查询 SPI | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、Sandbox/签名、其余 7.x、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M316、M317、M273、M297～M302 |
...
审核回调、取消/更新入站、Profile/Route 注册表（OpenAPI **1.0.41**；Flyway **118 / 120**）。

下一主线（已批准）：吉利 Sandbox/OpenAPI 签名真实联调（BLOCKED_EXTERNAL）；人工标记已送达/放弃 →
