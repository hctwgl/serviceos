...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M356 客户端能力兼容发布门禁 |
| 基线提交 | `PENDING_MERGE`（合并后回填） |
...
| 契约 | Core OpenAPI 1.0.50 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、`workorder.external-details-updated@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1；ConfigurationDraft.clientCompatibility） |
...
|---|---|---|---|---|---|
| 工程基础 | 构建、测试、契约、可观测性、容器发布 | `IMPLEMENTED` | Maven、PostgreSQL IT、契约门禁、Trace/指标、单镜像迁移和回滚演练；Track A 同源 TS/Swift Client、Token、Web/iOS Core、身份注册和有界客户端元数据；M256/M257 独立 Web；M258～M260 iOS Foundation/App/Simulator；M261 Production archive/dSYM、隐私/AppIcon 与签名失败关闭；**M356** FORM/EVIDENCE 发布客户端能力兼容门禁与分端报告 | 签名真机/TestFlight、灰度定向发布、运行时拒单、生产 IdP/BFF、正式 K8s、多故障域、PITR、远端制品发布/SBOM/签名、正式 Secret Manager | M8～M14、M247～M261、M356 |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M350** 配置驱动履约主链路（含 Mapping DSL Admin UI + DISPATCH 编辑器 + FORM/EVIDENCE ConditionBuilder + Technician 条件/规则执行器）；**M356** FORM/EVIDENCE 客户端能力兼容发布门禁与设计器报告 | AMOUNT/加权（待确认）、BUSINESS 日历 SLA、editableWhen/defaultExpression、灰度/定向发布、运行时拒单、iOS 条件全量硬阻断 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M350、M356 |
...
ServiceOS 可靠纵向切片已推进到 **M356**（客户端能力兼容发布门禁；承接 M355 终审工作台）。
OpenAPI **1.0.50**（`ConfigurationDraft.clientCompatibility`）；Flyway **130**。

下一阶段：

1. 客户端能力协商后续：灰度/`supportedClientKinds`、任务详情运行时拒单、iOS 条件执行器对齐后的全量硬阻断；
