...
| 契约 | Core OpenAPI **1.0.71** + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `change-timeline`、`GET /org-memberships`、`recent-logins`、`POST /security-principals`、`GET /admin/user-directory`、`/me/followed-projects`、履约 Runbook/Compare-Impact 等既有契约） |
...
| Admin Portal | 总部运营后台 | `PARTIAL` | **M284/M287/M289/M291/M292/M294/M295/M296** 配置设计器；M101～M193 运营基线；**M328** UNKNOWN；**M351** 终审；**M370～M377** 设计系统；**M384～M389** 配置四核与工单详情；**M397～M405** 用户管理登记/摘要/最近登录/组织任职/变更时间线、项目管理与关注项目（`READY_FOR_REVIEW`） | 行政区名称树/车企主数据、跨聚合业务时间线、正式企业 OIDC/BFF；不得宣称 PRODUCT_ACCEPTED | M7 设计、M101～M193、M284～M296、M328、M351、M370～M377、M384～M389、M397～M405、Admin 试点基线 |
...
2. 完整行政区名称树 / 独立车企主数据；
3. 跨聚合业务时间线（任职/RoleGrant）与操作者显示名。
