...
baselineCommit: 20beffdab01445e109ef9c61b57b1b9f5262bd8a
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M285 配置草稿 Diff 与发布审批 |
| 基线提交 | `20beffdab01445e109ef9c61b57b1b9f5262bd8a`（功能证据；合并入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 113 / 115） |
| 契约 | Core OpenAPI 1.0.30 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器草稿与 Admin 壳 | `PARTIAL` | M282～M284 + **M285** Diff/审批门禁（VALIDATED→APPROVED→PUBLISHED，OpenAPI 1.0.30） | 拖拽画布、灰度/回滚、多级审批、Bundle UI | M16、M33、M36、M52～M53、M61、M268、M271、M281～M285 |
...
ServiceOS 可靠纵向切片已推进到 **M285**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS、多实例、取消/重开/跳转，取消时配置化补偿任务（Flyway V109），标准模板族，配置设计器 API，Admin 设计器壳，以及 **Diff/审批门禁**（Flyway V112～V113）。
