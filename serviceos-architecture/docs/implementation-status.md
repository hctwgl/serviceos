...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M85 工单工作区只读组合快照 |
| 基线提交 | `PENDING_M85`（L3 verify 后回填） |
...
| 应用只读投影 | 工作区、队列、时间线和投影运行时 | `PARTIAL` | 独立 readmodel 模块；核心执行、现场履约、SLA、资料/审核/整改（含外部回执与条件 KEEP/INVALIDATE 处置）、外发交付全链路、异常确认/闭环、ServiceAssignment 与 Task 指派/Guard/人工接管 Inbox 投影；授权时间线与稳定分页；时间线 checkpoint/dead letter/generation 重建与 FRESH/LAGGING/UNKNOWN/REBUILDING freshness；工单工作区顶层实时组合（无 PII；缺权 SLA/异常降级） | 试算合并、revision/slots 技术噪声、sections 按需加载、队列、SavedView、搜索、多投影平台、Broker offset、Portal | M73～M85 |
...
ServiceOS 可靠纵向切片已推进到 **M85**。M61～M85 在授权只读、时间线投影与工作区组合上继续收敛；
...
8. 履约事实试算与结算运行时；
9. DATA-06 投影平台后续（definition 表、dead-letter 重放、旧 generation 清理）。
```
