...
baselineCommit: b06bca3fd6d7971841b2aabf45b67552db1393ad
latestMilestone: M243
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M243 Technician Portal 当前责任任务在线详情 |
| 基线提交 | `b06bca3fd6d7971841b2aabf45b67552db1393ad` |
...
| 契约 | Core OpenAPI 1.0.17 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| Technician Portal 当前责任任务详情 | ACTIVE 当前师傅任务在线非 PII 详情 | `IMPLEMENTED` | `GET /technician/me/tasks/{taskId}`；可信 Technician 上下文 + NETWORK `task.readAssigned` + ACTIVE ServiceAssignment/TaskAssignment 门禁；Task 头/guard/version/预约摘要；Feed 深链；OpenAPI 1.0.17；Flyway 100/102 | 联系/预约、Visit、表单、Evidence、整改写命令；离线工作包、GPS/上传、MESSAGE/PROFILE、PII | M243 |
...
| Technician App / Portal | 师傅移动端与 Feed | `PARTIAL` | M195：Feed/schedule/sync-summary；`X-Technician-Context`；Admin Web `/technician-portal/*` shell；tombstone 与跨网点失败关闭；M243：ACTIVE 当前责任任务在线非 PII 详情与预约摘要 | 联系/预约、Visit、表单、Evidence、整改写命令；离线工作包、GPS/相机/大上传、真机与离线运行时；MESSAGE/PROFILE | M7 设计、M195、M243 |
...
ServiceOS 可靠纵向切片已推进到 **M243**。身份治理 M183～M188、Admin 个人/共享 SavedView、Admin UI
...
1. Technician Portal 在线履约写切片（联系/预约 → Visit → 表单/Evidence → 整改）与 MESSAGE / PROFILE / 离线 Sync Center 需逐项接受契约；
