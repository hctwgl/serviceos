...
baselineCommit: 9579d01220954fa301b1f0dbe32cabf59990a18b
latestMilestone: M288
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M288 CANARY 百分比流量灰度 |
| 基线提交 | `9579d01220954fa301b1f0dbe32cabf59990a18b`（功能证据；合并入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M287 + **M288** CANARY trafficPercent 哈希分流（OpenAPI 1.0.32） | 画布建边/条件编辑、多 CANARY、自动晋级 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M288 |
...
ServiceOS 可靠纵向切片已推进到 **M288**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS、多实例、取消/重开/跳转，取消时配置化补偿任务（Flyway V109），标准模板族，配置设计器 API，Admin 设计器壳，Diff/审批门禁，Bundle 灰度/回滚，Workflow 拖拽画布，以及 **百分比流量灰度**（Flyway V116）。
