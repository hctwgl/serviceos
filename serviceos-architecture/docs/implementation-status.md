...
baselineCommit: 25d59c6ebbd202c31964bdaf91e5b805eb7fb59d
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M323 ASSIGNEE_POLICY 自动接入 TaskAssignment |
| 基线提交 | `25d59c6ebbd202c31964bdaf91e5b805eb7fb59d`（功能证据；合入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 121） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321** 入站 Mapping 物化 + **M322** 出站 Mapping 生成提审 Payload + **M323** ASSIGNEE_POLICY → TaskAssignment CANDIDATE | 全量字段仅 Mapping、删除 Profile 硬编码、默认值/枚举/条件 DSL、DISPATCH/RULE/NOTIFICATION/PRICING/Admin 自动接入、嵌套表达式、DISPATCH 地图、BUSINESS 日历 SLA、结算落账 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321、M322、M323 |
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI；**M311/M314/M316/M320** 吉利本地 + 三 OEM 并行冒烟 + **M317～M319** 远端查询/人工处置/批量重放；**M321/M322** 冻结 INBOUND/OUTBOUND Mapping 主路径 | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、完全移除 OEM 兼容 Mapper/Profile payload、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M316、M317、M318、M319、M320、M321、M322、M273、M297～M302 |
...
ServiceOS 可靠纵向切片已推进到 **M323**（ASSIGNEE_POLICY 自动接入 TaskAssignment）。
M321～M323 均在 Draft stacked PRs 中；M320 已合入 `master`（`32b902f8`）。

当前契约/迁移：OpenAPI **1.0.43**；Flyway **121**。

下一阶段继续 **Configuration-Driven Fulfillment Runtime**：

1. **DISPATCH** 自动接入 ServiceAssignment + 容量预占；
2. **RULE** 接入 Evidence/Task/Review 等真实业务门禁；
3. **NOTIFICATION** 可靠投递闭环（Outbox/Attempt/ACK）；
4. **PRICING** 履约事实与 CalculationSnapshot；
5. **UNKNOWN / Replay Admin 工作台**；
6. 吉利真实联调材料到位后提升为最高优先级；否则保持 `BLOCKED_EXTERNAL`；
7. 低代码高级能力仅在对应运行时已接入业务主链路后深化。
