...
baselineCommit: e0ded347289f948f8d2998886c9deb1ab8694a31
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M304 建单 INTEGRATION Mapping 闸门 |
| 基线提交 | `e0ded347289f948f8d2998886c9deb1ab8694a31`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303/M304** INTEGRATION Mapping 运行时并接入建单闸门 | 条件积木、指标驱动晋级、其余资产运行时、出站 Mapping、替代 OEM Mapper | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303、M304 |
...
ServiceOS 可靠纵向切片已推进到 **M304**。M267～M296 基线已稳定；**M297～M302** 已交付出站提审、
审核回调、取消/更新入站、Profile/Route 注册表（OpenAPI **1.0.40**；Flyway **118 / 120**）。

下一主线（已批准）：P2 配置资产运行时（ASSIGNEE_POLICY → DISPATCH → …）→ 业务低代码设计器 →
真实第二家车企。远端查询 SPI 与 Track F/G 仍为 `BLOCKED_EXTERNAL`。

```text
已接受推进顺序：
1. P2 续：ASSIGNEE_POLICY → DISPATCH → NOTIFICATION → RULE → PRICING 运行时；
2. P1 余量：远端状态查询 SPI；
