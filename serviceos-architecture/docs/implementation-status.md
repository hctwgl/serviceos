...
baselineCommit: 7cec40b3e3662e332848a53ac74ff224aa9cecd5
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M322 出站 INTEGRATION Mapping 生成提审 Payload |
| 基线提交 | `7cec40b3e3662e332848a53ac74ff224aa9cecd5`（功能证据；合入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 120 / 122） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321** 入站 Mapping 物化 + **M322** 出站 Mapping 生成提审 Payload | 全量字段仅 Mapping、删除 Profile 硬编码、默认值/枚举/条件 DSL、运行时自动接入 TaskAssignment/ServiceAssignment/规则门禁/通知投递/履约试算、嵌套表达式、DISPATCH 地图、BUSINESS 日历 SLA、结算落账 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321、M322 |
...
ServiceOS 可靠纵向切片已推进到 **M322**（出站 INTEGRATION Mapping）。
M321 入站物化与 M322 出站 Mapping 均在 Draft PR 中；M320 已合入 `master`（`32b902f8`）。

当前契约/迁移：OpenAPI **1.0.43**；Flyway **120 / 122**。

下一阶段继续 **Configuration-Driven Fulfillment Runtime**：

1. **ASSIGNEE_POLICY** 自动接入 TaskAssignment；
2. **DISPATCH** 自动接入 ServiceAssignment + 容量预占；
3. **RULE** 接入 Evidence/Task/Review 等真实业务门禁；
4. **NOTIFICATION** 可靠投递闭环（Outbox/Attempt/ACK）；
5. **PRICING** 履约事实与 CalculationSnapshot；
6. **UNKNOWN / Replay Admin 工作台**；
7. 吉利真实联调材料到位后提升为最高优先级；否则保持 `BLOCKED_EXTERNAL`；
8. 低代码高级能力仅在对应运行时已接入业务主链路后深化。
