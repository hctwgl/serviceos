...
baselineCommit: da6721b829284330c810ea9d0cd95d090ce5823b
latestMilestone: M48
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M48 ReviewCase 强制通过与重开 |
| 基线提交 | `da6721b829284330c810ea9d0cd95d090ce5823b` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts` |
| 前端工程 | 尚未建立；已有 Admin、Network、Technician 产品与交互规格 |
| 数据库 | PostgreSQL + Flyway（当前版本 048 / 50） |
| 契约 | OpenAPI 0.23.0 + 事件 JSON Schema |
...
| 资料 Evidence | 资产、槽位、Item/Revision、机器校验、Snapshot、完成门禁、作废、Review、Correction | `PARTIAL` | 固定槽位、安全文件 Finalize/Invalidate 联动、确定性机器校验、TASK_SUBMISSION Snapshot、完成门禁、作废、Review/Correction/整改 Task、强制通过/重开 | 条件槽位、OCR/CV | M36～M48 |
...
| 审核整改 | ReviewCase、ReviewDecision、CorrectionCase | `PARTIAL` | Review + Correction + 整改 Task + 强制通过/重开；补传轮次只追加 | 车企回执、自动指派、前端 | M44～M48 |
...
```text
M49 条件槽位 requiredWhen（需接受 ADR-018）或车企回执
```
