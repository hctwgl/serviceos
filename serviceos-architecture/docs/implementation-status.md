...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M390 Network 工作台与分配师傅产品化（承接 Network 黄金链路） |
...
| Admin Portal | 总部运营后台 | `PARTIAL` | **M284/M287/M289/M291/M292/M294/M295/M296** 配置设计器；M101～M193 运营基线；**M328** UNKNOWN 人工确认/放弃与批量 Replay；**M351** 终审工作区；**M370～M377** 设计系统/AppShell；**M384** 产品蓝图与经典专业风基线；**M385/M388** 履约配置中心与结构化 Draft；**M386** 工作流设计器；**M387** 任务模板中心；**M389** 工单详情履约工作区（`READY_FOR_REVIEW`） | 工作台/用户/项目母版深化、正式企业 OIDC/BFF、批量压测/MFA；不得宣称 PRODUCT_ACCEPTED | M7 设计、M101～M193、M284～M296、M328、M351、M370～M377、M384～M389、Admin 试点基线 |
| Network Portal | 网点协作端 | `PARTIAL` | M194～M242 已交付 workbench、工单/任务、限定工作区、师傅关系/资质、产能、整改和运营异常等受控读写切片；M256 将全部正式产品页迁入独立 `serviceos-network-web`；**M390** 工作台经典专业协作风 + SummaryStrip + 待分配表 + 分配师傅抽屉（`READY_FOR_REVIEW`） | 师傅个人负载/冲突正式读模型、工作区抽屉联动、评分引擎、Portal ACK/resolve/decide、notifications、生产 IdP | M7 设计、M194～M242、M255～M256、M390 |
...
1. Network 工单工作区 / 预约协同产品化；
2. Technician H5 在线闭环；
