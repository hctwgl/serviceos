...
baselineCommit: e12cac11fc7d7ac7c127bb4982b16f7283afa0f2
latestMilestone: M313
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M313 FORM/EVIDENCE/SLA 可视配置器 |
| 基线提交 | `e12cac11fc7d7ac7c127bb4982b16f7283afa0f2`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 118 / 120） |
| 契约 | Core OpenAPI 1.0.40 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、`workorder.external-details-updated@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M313** 条件积木/画布/FORM·EVIDENCE·SLA 可视编辑 | 嵌套表达式 round-trip、RULE/DISPATCH 全属性面板、出站 Mapping、真实短信/邮件 Adapter、结算落账 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M313 |
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI（建单、更新、取消、出站、回调、Profile/Route）；**M311** 吉利 AES+7.1 本地建单 | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、7.2～7.22、远端查询 SPI、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M273、M297～M302 |
...
下一主线（已批准）：P3 余量（RULE/DISPATCH 属性面板）或 P4 吉利 7.2+ 本地 stub；Sandbox 仍 BLOCKED_EXTERNAL →
