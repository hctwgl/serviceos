...
| 数据库 | PostgreSQL + Flyway（当前版本 135 / 137） |
...
ServiceOS 后端可靠纵向切片基线仍为 **M369**（BUSINESS 日历 SLA 截止时间 / ADR-090 D1-R/D2-R/D4-R）。
OpenAPI **仍 1.0.59**；Flyway **135**。
**Admin Web 产品化轨道**已推进到 **M377**（M370～M377：设计系统 Presenter、AppShell、标准模板、工单中心/详情、项目详情、主导航迁移与视觉关闭；不改后端契约）。

**后端下一切片状态：`AWAITING_OWNER_SELECTION`** — 交接文档候选 A–F 仍需负责人选型（与已完成的 Admin 前端 M370～M377 并行轨道，互不覆盖）。

下一阶段候选（均需负责人确认后才能推进，不得发明规则）：

1. Admin 产品化深化：剩余队列 Ant Table 化、全量视觉态、真实 OIDC 冒烟与 a11y 人工旅程补齐；
2. iOS 条件执行器全量硬阻断（本 Linux 环境多为 BLOCKED_EXTERNAL）；
3. 吉利联调 / AMOUNT/加权（硬门禁）；
4. BUSINESS 暂停/恢复/预警/升级（需另接受 ADR-090 D1-B 或 ARCH-12 子集）；
5. OpenAPI 放宽 `clockMode`/`CALENDAR`（闭合 M369 HTTP 契约缺口；需批准破坏性演进或新主版本）。
