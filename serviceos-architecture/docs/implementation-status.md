...
| 前端工程 | `serviceos-admin-web`（Vue+TS+Vite）已纳入 CI 构建，具备开发态 Keycloak PKCE，以及真实只读、Task MANUAL assign-candidates/claim/release、锁定表单提交和独立 Task complete 至 WorkOrder FULFILLED 的局部写链路 PR 阻断 E2E；Network/Technician 尚未建立 |
...
| Admin Portal | 总部运营后台 | `PARTIAL` | M101～M134：队列/任务/SLA/异常/外发/工单/项目目录、工作区、allowed-actions 命令面板；CI 阻断构建；开发态 Keycloak PKCE；真实只读、Task MANUAL assign-candidates/claim/release、锁定表单提交与独立 Task complete 至 WorkOrder FULFILLED 的局部写链路 PR 阻断 E2E | 设计系统、SavedView、正式企业 OIDC/BFF、完整履约写链路 E2E | M7 设计、M101～M134、Admin 试点基线 |
...
- 真实 Backend/PostgreSQL/Chrome 的 E2E，覆盖目录、工作区、详情、Stage、Task、SLA、时间线，
  ...
  assign/claim/start、锁定 FormVersion 提交、精确 submission ref/digest complete、
  Outbox/Inbox 消费与 WorkOrder FULFILLED；
...
2. 用真实脱敏试点业务数据验证接单至完结的完整履约写链路；当前 Admin 基线只证明只读投影与
   Task MANUAL assign-candidates/claim/release，以及独立预置 Workflow Task 的表单提交/complete
   至 WorkOrder FULFILLED 局部写链路；
