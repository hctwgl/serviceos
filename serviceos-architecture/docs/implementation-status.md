...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M408 Network 分配候选预约日程冲突摘要 |
...
| 契约 | Core OpenAPI **1.0.74** + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `/project-clients`、`/region-catalog`、`change-timeline`、`org-memberships`、`recent-logins`、用户目录与关注项目等既有契约） |
...
| Network Portal | 网点协作端 | `PARTIAL` | M194～M242 读写切片；M256 独立 `serviceos-network-web`；**M390～M392** 工作台/工作区/整改异常；**M396** 师傅与产能；**M407～M408** 分配候选与日程冲突摘要（`READY_FOR_REVIEW`） | 距离读模型、产能申请写、评分引擎、Portal ACK/resolve/decide、notifications、生产 IdP | M7 设计、M194～M242、M255～M256、M390～M392、M396、M407～M408 |
