...
baselineCommit: 579fd33b19992b0d4f5dcf9b5df1cb2ac38dd910
latestMilestone: M252
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M252 多端 Page/Feature/Action 机器注册表 |
| 基线提交 | `579fd33b19992b0d4f5dcf9b5df1cb2ac38dd910` |
...
|---|---|---|---|---|---|
| 工程基础 | 构建、测试、契约、可观测性、容器发布 | `IMPLEMENTED` | Maven、PostgreSQL IT、契约门禁、Trace/指标、单镜像迁移和回滚演练；同源 TypeScript/Swift Client；单源 Design Token；无角色 Web/iOS auth/context/error/trace Core；Page/Feature/Action 单源注册与未知动作失败关闭 | 独立客户端实际导入、clientKind/clientVersion、生产 OIDC/BFF、Keychain/Xcode/真机、正式 K8s、多故障域、PITR、制品发布/SBOM/签名、正式 Secret Manager | M8～M14、M247～M252 |
...
ServiceOS 可靠纵向切片已推进到 **M252**。身份治理 M183～M188、Admin 个人/共享 SavedView、Admin UI
...
Page/Feature/Action 单源机器注册、Web/Swift 生成与未知动作失败关闭基础。独立页壳剩余主要是
