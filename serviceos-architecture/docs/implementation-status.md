...
baselineCommit: 833551f2a22a5c72711d7eab7ec77f8325c2d273
latestMilestone: M316
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M316 吉利提审出站 stub 与审核回调 |
| 基线提交 | `833551f2a22a5c72711d7eab7ec77f8325c2d273`（功能证据提交后回填；合入 `master` 后改为合并提交） |
...
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI（建单、更新、取消、出站、回调、Profile/Route）；**M311/M314/M316** 吉利 AES 本地建单/更新/取消/提审 stub/审核回调 | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、Sandbox/签名、其余 7.x、远端查询 SPI、人工标记已送达/放弃、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M316、M273、M297～M302 |
...
下一主线（已批准）：吉利 Sandbox/OpenAPI 签名真实联调（BLOCKED_EXTERNAL）；远端查询 SPI →
