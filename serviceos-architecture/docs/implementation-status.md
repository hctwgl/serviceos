...
baselineCommit: "b06341a2db7007b55fe82d69872a46e4dae44c99"
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M336 CREATE_WORK_ORDER RouteHint 与 OEM 字段 Mapper 瘦身 |
| 基线提交 | `b06341a2db7007b55fe82d69872a46e4dae44c99`（功能证据；合入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M336** 配置驱动履约主链路（含 Mapping DSL/强制/RouteHint + DISPATCH NETWORK/TECHNICIAN） | Update/Cancel Mapping 强制、嵌套表达式、DISPATCH 地图/比例分配、BUSINESS 日历 SLA、结算落账、低代码深化 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M336 |
...
ServiceOS 可靠纵向切片已推进到 **M336**（CREATE_WORK_ORDER RouteHint）。
M321～M336 均在 Draft stacked PRs 中（`#148→…→#163`；本切片 https://github.com/hctwgl/serviceos/pull/163）；M320 已合入 `master`（`32b902f8`）。
