...
baselineCommit: fcc090cf9dc21545bf46488b2d0e1b05803974c1
latestMilestone: M259
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M259 Technician iOS SwiftUI App 与 Xcode 交付批次 |
| 基线提交 | `fcc090cf9dc21545bf46488b2d0e1b05803974c1` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts`、`@serviceos/web-core`、`ServiceOSIOSCore`、独立且可部署的 `serviceos-network-web` 与 `serviceos-technician-web`、Swift 6 `TechnicianIOSFoundation`，以及可由 Xcode 构建 generic Simulator/iPhoneOS 产物和测试 bundle 的原生 `TechnicianIOS` SwiftUI App；由同一 Core OpenAPI 生成并经独立消费者门禁验证的 `@serviceos/core-client` 与 `ServiceOSCoreClient` |
...
|---|---|---|---|---|---|
| 工程基础 | 构建、测试、契约、可观测性、容器发布 | `IMPLEMENTED` | Maven、PostgreSQL IT、契约门禁、Trace/指标、单镜像迁移和回滚演练；Track A 同源 TS/Swift Client、Token、Web/iOS Core、身份注册和有界客户端元数据；M256/M257 独立 Web 实际导入；M258 Foundation；M259 Xcode SwiftUI App、generic Simulator/iPhoneOS 与测试 bundle 构建 | Simulator 运行态、签名/真机/TestFlight、能力协商、生产 IdP/BFF、正式 K8s、多故障域、PITR、远端制品发布/SBOM/签名、正式 Secret Manager | M8～M14、M247～M259 |
...
| Technician App / Portal | 师傅移动端与 Feed | `PARTIAL` | M195/M218/M219/M243～M246 只读安全切片；M257 独立移动优先 H5；M258 iOS 安全 Foundation；M259 原生 SwiftUI AppShell、系统 OIDC、Keychain 会话恢复、TECHNICIAN Context/Capability/导航、五环境 Xcode 工程、Simulator/iPhoneOS 与测试 bundle 构建 | Simulator 测试运行、签名真机/真实 IdP/VoiceOver/TestFlight；Track E 在线写命令、Evidence、提交/整改；Track F 离线工作包/后台上传；GPS/相机；MESSAGE/PROFILE | M7 设计、M195、M218～M219、M243～M246、M257～M259 |
...
ServiceOS 可靠纵向切片已推进到 **M259**。身份治理 M183～M188、Admin 个人/共享 SavedView、Admin UI
...
SwiftUI App、五环境工程、generic Simulator/iPhoneOS 构建和测试 bundle。下一批次仍属于 Track D，必须
在用户手动启动的 Simulator 上实跑 XCTest/XCUITest，并补齐签名真机、真实 IdP、VoiceOver 与 TestFlight；
不能用 build-for-testing 或无签名 device build 替代。Network 独立页壳剩余主要是
