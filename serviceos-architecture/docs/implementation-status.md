...
baselineCommit: 3b753e74f8dfedd0f070fdf9d0b4f54a8baa5f6e
latestMilestone: M275
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M275 PARALLEL_GATEWAY 分叉与汇聚运行时 |
| 基线提交 | `3b753e74f8dfedd0f070fdf9d0b4f54a8baa5f6e`（功能证据；合并入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 102 / 104） |
...
| 工作流 | 线性 + EXCLUSIVE/PARALLEL 网关 + WAIT_EVENT | `PARTIAL` | 线性推进；**M269** EXCLUSIVE；**M270** WAIT_EVENT；**M275** PARALLEL fork/join 汇聚计数 | 定时器/超时、子流程、多实例、撤回/取消/重开 | M17～M19、M61、M69、M268～M270、M275 |
...
ServiceOS 可靠纵向切片已推进到 **M275**。阶段一已闭合；**M275** 已交付 PARALLEL_GATEWAY
分叉激活与 join 汇聚计数运行时（Flyway V102）。

下一主线：定时器/超时事件 → 子流程 → 多实例 → 撤回/取消/重开；其后阶段三配置设计器。
真实 OEM2/3 与签名真机仍为 `BLOCKED_EXTERNAL`。

```text
已接受推进顺序：
1. M276 定时器/超时事件；
2. 子流程 / 多实例 / 撤回取消重开 / 异常补偿；
3. 安装/维修/移机/巡检标准模板；
4. 阶段三领域配置设计器；
