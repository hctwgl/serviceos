...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M80 工单服务分配生命周期时间线事件合并 |
| 基线提交 | `PENDING_M80` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts` |
| 前端工程 | 尚未建立；已有 Admin、Network、Technician 产品与交互规格 |
| 数据库 | PostgreSQL + Flyway（当前版本 076 / 78） |
| 契约 | Core OpenAPI 0.51.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 应用只读投影 | 工作区、队列、时间线和投影运行时 | `PARTIAL` | 独立 readmodel 模块；核心执行、现场履约、SLA、资料/审核/整改、外发交付全链路、异常确认/闭环与 ServiceAssignment 激活生命周期 Inbox 投影；授权时间线与稳定分页 | 试算合并、revision 技术噪声、工作区/队列、SavedView、搜索、重建、checkpoint、Portal | M73～M80 |
...
ServiceOS 可靠纵向切片已推进到 **M80**。M61～M80 在授权只读与时间线投影上继续收敛现场履约、SLA、
资料审核与派单可见性；没有实现完整 SLA/通知策略或整个现场履约平台。
