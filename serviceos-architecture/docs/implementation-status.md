...
baselineCommit: 835f03ec2c31f67f152413e076c2e8f7360391e8
latestMilestone: M278
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M278 多实例任务运行时 |
| 基线提交 | `835f03ec2c31f67f152413e076c2e8f7360391e8`（功能证据；合并入 `master` 后改为合并提交） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 105 / 107） |
...
| 工作流 | 线性 + 网关 + WAIT/TIMER + SUB_PROCESS + 多实例 | `PARTIAL` | 线性；EXCLUSIVE；WAIT；PARALLEL；TIMER；SUB_PROCESS；**M278** multiInstance.cardinality | 撤回/取消/重开、集合驱动 cardinality、异常补偿 | M17～M19、M61、M69、M268～M270、M275～M278 |
...
ServiceOS 可靠纵向切片已推进到 **M278**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS 与 **多实例任务**（Flyway V105）。

下一主线：撤回/取消/重开/异常补偿 → 标准模板扩展 → 阶段三配置设计器。
真实 OEM2/3 与签名真机仍为 `BLOCKED_EXTERNAL`。

```text
已接受推进顺序：
1. M279 撤回/取消/重开（或人工跳转）最小切片；
2. 异常补偿；
3. 安装/维修/移机/巡检标准模板；
4. 阶段三领域配置设计器；
5. Track F/G 与真实 OEM 外部证据。
```
