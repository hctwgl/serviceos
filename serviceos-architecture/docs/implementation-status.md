...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M358 supportedClientKinds 定向发布 |
...
| 契约 | Core OpenAPI 1.0.52 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 `workorder.cancelled@v1`、`workorder.reopened@v1`、`workorder.external-details-updated@v1`、project.created@v3、project.scope-relations-revised@v1、`task.handling-completed@v1`、recovered/resolved 与 SLA started/breached/met@v1；ConfigurationDraft.clientCompatibility/`supportedClientKinds`；`CLIENT_CAPABILITY_UNSUPPORTED`） |
...
|---|---|---|---|---|---|
| 工程基础 | 构建、测试、契约、可观测性、容器发布 | `IMPLEMENTED` | Maven、PostgreSQL IT、契约门禁、Trace/指标、单镜像迁移和回滚演练；Track A 同源 TS/Swift Client、Token、Web/iOS Core、身份注册和有界客户端元数据；M256/M257 独立 Web；M258～M260 iOS Foundation/App/Simulator；M261 Production archive/dSYM、隐私/AppIcon 与签名失败关闭；**M356～M358** 客户端能力发布门禁、运行时拒单与 supportedClientKinds 定向发布 | 签名真机/TestFlight、生产 IdP/BFF、正式 K8s、多故障域、PITR、远端制品发布/SBOM/签名、正式 Secret Manager | M8～M14、M247～M261、M356～M358 |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M350** 配置驱动履约主链路（含 Mapping DSL Admin UI + DISPATCH 编辑器 + FORM/EVIDENCE ConditionBuilder + Technician 条件/规则执行器）；**M356～M358** 客户端能力发布门禁、运行时拒单与 supportedClientKinds 定向发布 | AMOUNT/加权（待确认）、BUSINESS 日历 SLA、editableWhen/defaultExpression、iOS 条件全量硬阻断、派单/Feed 级目标过滤 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M350、M356～M358 |
...
| Technician App / Portal | 师傅移动端与 Feed | `PARTIAL` | M195/M218/M219/M243～M246 只读安全切片；M257 独立 H5；M258～M261 iOS 基础；M262 在线 Visit；M263 冻结基础表单；**M349/M350** H5 条件执行器 + 工单/区域权威头 + validationRules；M264 Evidence 采集上传；M265 Snapshot 与 Task 完成；M266 在线整改；**M357～M358** 运行时能力拒单与定向目标外拒单 | 联系/预约、完整表单草稿、editableWhen/默认值（未接受）、iOS 共用执行器、真实 operationRefs 签退；弱网/后台/Track F 离线；签名真机/真实 IdP/VoiceOver/崩溃采集/TestFlight | M7 设计、M195、M218～M219、M243～M246、M257～M266、M349、M350、M357、M358 |
...
ServiceOS 可靠纵向切片已推进到 **M358**（supportedClientKinds 定向发布；承接 M356/M357）。
OpenAPI **1.0.52**；Flyway **131**。

下一阶段：

1. 客户端能力协商后续：iOS 条件执行器对齐后的全量硬阻断、Feed/详情头级拒单、派单过滤；
