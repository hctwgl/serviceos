...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M84 工单时间线投影 Checkpoint 与重建 |
| 基线提交 | `PENDING_M84` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts` |
| 前端工程 | 尚未建立；已有 Admin、Network、Technician 产品与交互规格 |
| 数据库 | PostgreSQL + Flyway（当前版本 077 / 79） |
| 契约 | Core OpenAPI 0.55.0 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| 应用只读投影 | 工作区、队列、时间线和投影运行时 | `PARTIAL` | 独立 readmodel 模块；核心执行、现场履约、SLA、资料/审核/整改（含外部回执与条件 KEEP/INVALIDATE 处置）、外发交付全链路、异常确认/闭环、ServiceAssignment 与 Task 指派/Guard/人工接管 Inbox 投影；授权时间线与稳定分页；时间线 checkpoint/dead letter/generation 重建与 FRESH/LAGGING/UNKNOWN/REBUILDING freshness | 试算合并、revision/slots 技术噪声、工作区/队列、SavedView、搜索、多投影平台、Broker offset、Portal | M73～M84 |
...
ServiceOS 可靠纵向切片已推进到 **M84**。M61～M84 在授权只读、时间线投影与投影运行时上继续收敛；
没有实现完整 SLA/通知策略、工作区/队列或整个现场履约平台。

```text
候选下一方向（优先从已确认文档中选择最小可靠切片）：
1. 在 API-06/M7 接受后继续工作区、队列、SavedView 与 Portal 只读体验；
