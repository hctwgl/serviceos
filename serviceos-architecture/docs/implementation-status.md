...
baselineCommit: "22bea256c07b4533949fc6738e572e20928e6417"
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M335 CREATE_WORK_ORDER 强制 INBOUND Mapping |
| 基线提交 | `22bea256c07b4533949fc6738e572e20928e6417`（功能证据；合入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M335** 配置驱动履约主链路（含出站/入站仅 Mapping + DSL + CREATE 强制 Mapping + DISPATCH NETWORK/TECHNICIAN） | OEM Java Mapper 拆除、嵌套表达式、DISPATCH 地图/比例分配、BUSINESS 日历 SLA、结算落账、低代码深化 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M335 |
| 外部接入 | BYD CPIM + REFERENCE_OEM SAMPLE + Geely 本地切片 | `PARTIAL` | BYD 入站建单/更新/取消、提审、回调；**M267/M297～M302** 通用 SPI；**M311/M314/M316/M320** 吉利本地 + 三 OEM 并行冒烟 + **M317～M319** 远端查询/人工处置/批量重放；**M321/M322/M331/M333/M334/M335** 冻结 Mapping（出站仅 Mapping；入站无 fallback + DSL；CREATE 强制 INBOUND Mapping） | 吉利 Sandbox/OpenAPI 签名联调（BLOCKED_EXTERNAL）、入站完全移除 OEM 兼容 Mapper、生产凭据/对象存储 | M16、M56～M60、M77～M79、M99、M158、M267、M272、M311、M314、M316、M317、M318、M319、M320、M321、M322、M331、M333、M334、M335、M273、M297～M302 |
...
ServiceOS 可靠纵向切片已推进到 **M335**（CREATE_WORK_ORDER 强制 INBOUND Mapping）。
M321～M335 均在 Draft stacked PRs 中（`#148→…→#162`；本切片 https://github.com/hctwgl/serviceos/pull/162）；M320 已合入 `master`（`32b902f8`）。

当前契约/迁移：OpenAPI **1.0.43**；Flyway **125**（本切片无迁移）。

下一阶段继续 **Configuration-Driven Fulfillment Runtime**：

1. 删除 OEM 入站 Java 字段 Mapper / 引入 RouteHint DTO；
2. DISPATCH：地图 scope / 比例分配；
