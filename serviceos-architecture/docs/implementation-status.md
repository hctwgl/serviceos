...
baselineCommit: 47ea7ad4bbbde10d74244d5ea4d3f4fd7c4c11e7
latestMilestone: M287
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M287 Workflow 可视化拖拽画布 |
| 基线提交 | `47ea7ad4bbbde10d74244d5ea4d3f4fd7c4c11e7`（功能证据；合并入 `master` 后改为合并提交） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M286 + **M287** Workflow 拖拽画布（metadata.layout） | 画布建边/条件编辑、百分比流量灰度 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M287 |
...
ServiceOS 可靠纵向切片已推进到 **M287**。阶段一已闭合；复杂流程运行时已覆盖 EXCLUSIVE、PARALLEL、
WAIT_EVENT、TIMER、SUB_PROCESS、多实例、取消/重开/跳转，取消时配置化补偿任务（Flyway V109），标准模板族，配置设计器 API，Admin 设计器壳，Diff/审批门禁，Bundle 灰度/回滚，以及 **Workflow 拖拽画布**。
