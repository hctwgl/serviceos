...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M365 REVIEW_TASK 工作流门闸推进（A5-B） |
| 基线提交 | `PENDING_MERGE`（合并后回填；依赖 M364） |
...
| 数据库 | PostgreSQL + Flyway（当前版本 133） |
...
| 配置中心 | 不可变配置资产、Bundle 发布、设计器与灰度通道 | `PARTIAL` | M282～M296 设计器/治理 + **M303～M309** 六类运行时 + **M310～M315** 设计器 + **M321～M350** 配置驱动履约主链路（含 Mapping DSL Admin UI + DISPATCH 编辑器 + FORM/EVIDENCE ConditionBuilder + Technician 条件/规则执行器）；**M356～M363** 客户端能力发布门禁、运行时拒单、定向发布、Feed/详情头拒单、整改路径对齐、整改列表预检与领取/启动硬拒 | AMOUNT/加权（待确认）、BUSINESS 日历 SLA、editableWhen/defaultExpression、iOS 条件全量硬阻断、派单级目标过滤 | M16、M33、M36、M52～M53、M61、M268、M271、M281～M296、M303～M315、M321～M350、M356～M363 |
...
ServiceOS 可靠纵向切片已推进到 **M365**（REVIEW_TASK 工作流门闸 / A5-B；承接 M364）。
OpenAPI **1.0.57**；Flyway **133**。
Technician 能力门禁（M356～M363）、审核 handling Task（M364）与工作流门闸推进（M365）已闭环。

下一阶段（均需负责人确认后才能推进，不得发明规则）：

1. **派单级 `supportedClientKinds` 过滤** — 设计提案见 ADR-088 / M366（**Proposed**，待 Accept A1-R～A5-R）；未接受前不得实现；
