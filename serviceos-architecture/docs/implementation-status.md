...
| Admin Portal | 总部运营后台 | `PARTIAL` | M101～M134：队列/任务/SLA/异常/外发/工单/项目目录、工作区、allowed-actions 命令面板；CI 阻断构建；开发态 Keycloak PKCE；真实只读、Task MANUAL assign-candidates/claim/release、锁定表单提交、资料上传/校验/Snapshot、INTERNAL ReviewCase APPROVED、精确双引用与独立 Task complete 至 WorkOrder FULFILLED 的局部写链路 PR 阻断 E2E | 设计系统、SavedView、正式企业 OIDC/BFF、生产对象存储/专业扫描、审核驳回整改/外部提审、完整履约写链路 E2E | M7 设计、M101～M134、Admin 试点基线 |
...
未实现：正式企业 IdP/BFF、生产对象存储/专业扫描、审核驳回整改/强制通过/重开/外部提审，以及
接单至完结的完整履约写链路 E2E；详见
