...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M420 Admin 师傅客户端种类声明并入主体变更时间线 |
...
| 契约 | Core OpenAPI **1.0.86** + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 CLIENT_KINDS 时间线、authorization-denials、师傅档案/服务关系/网点任职 change-timeline、跨聚合 change-timeline、主数据治理、预约日历、分配候选推荐解释、工作台今日预约时间轴、分配候选距离亲和、关注项目角标、`/project-clients`、`/region-catalog`、`org-memberships`、`recent-logins`、用户目录等既有契约） |
...
3. 失败登录/设备指纹（当前仅成功登录事实）/ 工单侧跨域时间线深化。
