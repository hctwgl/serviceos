...
baselineCommit: b067af8dfe7697ce1100b25480ae80a9bfc44ba1
latestMilestone: M262
---
...
| 项目 | 当前值 |
|---|---|
| 最新实施里程碑 | M262 Technician 在线 Visit 交付批次 |
| 基线提交 | `b067af8dfe7697ce1100b25480ae80a9bfc44ba1` |
| 后端形态 | Java 21 + Spring Boot + Spring Modulith 模块化单体 |
| 当前可构建工程 | `serviceos-backend`、`serviceos-contracts`、`@serviceos/web-core`、`ServiceOSIOSCore`、独立且可部署的 `serviceos-network-web` 与 `serviceos-technician-web`、Swift 6 `TechnicianIOSFoundation`，以及已在 iPhone 17 Pro Simulator 安装启动、实跑 XCTest/XCUITest、形成 Production arm64 archive/dSYM，并接入当前任务与在线 Visit 的原生 `TechnicianIOS` SwiftUI App；由同一 Core OpenAPI 生成并经独立消费者门禁验证的 `@serviceos/core-client` 与 `ServiceOSCoreClient` |
| 前端工程 | `serviceos-admin-web` 独立承载总部运营、统一用户中心、`/me` 导航、SavedView、UI Preference、受控搜索与最近访问；M256 后 Network 正式产品由独立 `serviceos-network-web` 承载，M257 后 Technician Feed/Schedule/Sync/Me/Task Detail 由独立移动优先 `serviceos-technician-web` 承载，M262 增加一次性浏览器定位签到与中断；Admin 仅保留可配置外链和 M188 诊断；两套独立 Web 均实际接入 `@serviceos/web-core`、OIDC PKCE、服务端 Context/Capability/导航、Playwright 回归和独立容器镜像 |
| 数据库 | PostgreSQL + Flyway（当前版本 100 / 102） |
| 契约 | Core OpenAPI 1.0.22 + BYD CPIM OpenAPI 0.3.0 + 外部/事件 JSON Schema（含 project.created@v3、project.scope-relations-revised@v1、recovered/resolved 与 SLA started/breached/met@v1） |
...
| Technician Portal 当前责任任务详情 | ACTIVE 当前师傅任务在线非 PII 详情与协作历史 | `IMPLEMENTED` | 当前责任门禁；Task/预约/联系/Visit 安全摘要；project `form.read` 软门禁的表单提交校验摘要；Feed 深链；M262 H5/iOS 在线签到与中断；OpenAPI 1.0.22；Flyway 100/102 | 联系/预约写；动态表单/Evidence/真实 operationRefs 签退/整改；离线工作包、MESSAGE/PROFILE、PII | M243～M246、M262 |
...
| Technician Portal Feed | 师傅任务 Feed / 日程 / 同步摘要 / 我的 / 当前任务详情 | `IMPLEMENTED` | API-06 §11 子集；`X-Technician-Context`；本人 ACTIVE TECHNICIAN assignment Feed；tombstone；schedule；sync-summary；独立 H5/iOS；M218/M219/M243～M246 只读详情；M262 在线 Visit 签到/中断 | 离线工作包、mobile sync commands、MESSAGE、PROFILE、联系/预约/表单/Evidence/整改写闭环 | M195、M218、M219、M243～M246、M257～M262 |
...
| Technician App / Portal | 师傅移动端与 Feed | `PARTIAL` | M195/M218/M219/M243～M246 只读安全切片；M257 独立移动优先 H5；M258～M261 iOS Foundation/App/Simulator/分发基础；M262 H5/iOS 当前任务、一次性定位签到与中断 | 联系/预约、动态表单、Evidence、真实签退与整改在线闭环；Track F 离线；签名真机/真实 IdP/VoiceOver/崩溃采集/TestFlight | M7 设计、M195、M218～M219、M243～M246、M257～M262 |
...
ServiceOS 可靠纵向切片已推进到 **M262**。身份治理 M183～M188、Admin 个人/共享 SavedView、Admin UI
...
任务上的在线定位签到与无法施工中断，并以 Technician Context、当前责任和 Capability 双重失败关闭。由于
ADR-082 尚无权威联系人引用，联系/预约写未实施；由于尚无真实现场操作引用，客户端签退未开放。当前机器没有有效 Apple 签名身份或物理
iPhone，因此下一批 Track D/G 外部证据仍必须补齐开发团队签名真机、真实 IdP、VoiceOver 人工听读、崩溃
