...
1. 按 `serviceos-architecture/docs/agent-navigation.md` 路由表确定最小阅读集（3～6 个文件），用 `serviceos-architecture/docs/milestone-index.md` Grep 定位最近一次同模块里程碑；
2. 写下本次范围与**明确不做的边界**；
...
- 不要机械 `cat` 大文件或全量 Read 目录找上下文，回到 `serviceos-architecture/docs/agent-navigation.md` 路由；
