...
| M136 | Admin 预约/上门写链路 E2E：RoleGrant 写能力、ACTIVE ServiceAssignment 夹具、propose→confirm→check-in→check-out；Appointment/Visit COMPLETED 与审计/Inbox | PRODUCT-01/02 + ARCH-08/09/19 + Appointment/Visit APIs + Admin Web build/E2E + `149-m136-*` + `133-m136-*` + `admin-pilot-readiness-acceptance.md` | 用户确认渠道、GPS BLOCK、派单 Admin HTTP、外部提审 stub/回调、完整 ADMIN-PILOT-09 |
