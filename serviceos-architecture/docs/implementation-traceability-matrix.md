...
| M147 | Admin 工作区外发交付详情深链：INTEGRATION `outboundDeliveries[]` → `/integration/outbound/{id}`；复用已 Implemented GET/详情页；Playwright 断言 | PRODUCT-01/02 + ARCH-13/19 + API-06 workspace INTEGRATION + Outbound GET + Admin Web E2E + `160-m147-*` + `144-m147-*` | 专用入站队列列表 API、Review/Correction 队列筛选、SavedView、真实 sandbox |
