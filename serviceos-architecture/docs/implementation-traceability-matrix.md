...
| M134 | Admin 工单权威详情/Stage/Task/核心时间线；Admin 试点基线以真实 OIDC/Backend/PostgreSQL/Chrome PR 门禁覆盖 Task MANUAL assign/claim/release、表单/资料/APPROVED/双引用 complete 至 WorkOrder FULFILLED，以及独立 REJECTED→自动 CorrectionCase/整改 Task→CRITICAL WAIVED→整改 Task CANCELLED | PRODUCT-01/02 + ARCH-10/11/19 + Task/Form/Evidence/Review/Correction APIs + Workflow END Progression + Admin Web build/E2E + `admin-pilot-readiness-acceptance.md` | 正常整改补传/关闭/复审、强制通过/重开/外部提审、正式企业 OIDC/BFF、生产对象存储/专业扫描、从外部接单开始的完整履约写链路 E2E |
