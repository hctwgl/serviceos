...
| 履约方案 | FulfillmentPlan | 项目下一个可按结构化条件匹配的现场服务场景，独立维护版本与匹配优先级 | 单一工单类型或单一流程 |
| 履约方案版本 | FulfillmentPlanVersion | 履约方案的一次不可变版本，冻结适用范围/流程/SLA/派单/表单/证据/验收/通知/结算引用 | 项目级唯一配置版本 |
| 匹配优先级 | matchPriority | 多方案命中时按其降序、再按规则具体度确定唯一方案的整数 | 创建顺序或列表顺序 |
| 配置解析器 | ConfigurationResolver | 受理时先匹配唯一履约方案、再解析其生效版本对应配置包的能力 | 运行时猜测最近配置 |
