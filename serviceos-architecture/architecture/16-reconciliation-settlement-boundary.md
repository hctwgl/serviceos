...
`SHADOW`、`COMPARISON`、候选价格覆盖、authorityVersion 已过期或方向级计价权威尚未激活的 CalculationRun 永不具备正式结算资格；feature gate 不能把这些运行提升为权威结果。
...
开关至少按 tenant、项目、方向和 cohort 控制，并锁定生效版本。关闭时，正式结算命令返回 `FEATURE_NOT_ENABLED`，查询只允许访问已存在的迁移只读快照；不能以管理员身份、内部接口或直接调用领域服务绕过。
