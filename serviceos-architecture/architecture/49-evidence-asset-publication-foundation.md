...
1. M37 已按 Task 冻结 Bundle 和 Stage 解析固定 EvidenceSlot；M52 已实现基于工单/区域/任务
   权威事实的条件初次解析；字段版本冻结与条件变化重解析仍未实现；
2. M38 已对接安全文件 Begin/Finalize 并创建不可变 EvidenceItem/Revision；OCR/业务校验与 invalidate 仍未实现；
3. 创建并校验不可变 EvidenceSetSnapshot；
4. 在 ADR-008 获批后实现 ReviewDecision、CorrectionCase 和多轮补传闭环。
