...
> BYD 创建工单与审核回调已实现 `InboundEnvelope`/`CanonicalMessage`。
> BYD 提审已实现不可变 `OutboundDelivery`/`DeliveryAttempt`/
> `ExternalAcknowledgement`、Task 唯一重试时钟和 UNKNOWN 人工接管的最小纵向切片。
> UNKNOWN Delivery 支持单笔授权人工重发、不可变 ReplayRequest 与版本/审批门禁；
> 严格 ACK 后发布恢复事实，并幂等闭环对应运营异常及处理乱序事件。
> CREATE_WORK_ORDER 使用通用 `integration.spi` 与 `InboundCreateWorkOrderPipeline`，
> BYD 入站委托该管道。REFERENCE_OEM SAMPLE 入站 Connector 经同一管道，明确标记
> `TBD_EXTERNAL_CONTRACT`。出站提审使用 `OutboundSubmissionConnector` 与
> `OutboundSubmissionPipeline`，BYD 提审委托该管道；技术 ACK 与业务 ACK 在模型上分离。
> 审核回调使用逐单 `InboundReviewCallbackItemPipeline`（`ReviewCallbackMappedItem`），
> BYD 回调适配器保留验签/防重放/batch Envelope。入站取消/更新管道、BYD
> cancel-orders/update-orders、出站创建与 Route 的 connectorVersion 注册表已经存在。
> 远端查询 SPI、人工处置和批量 ReplayRequest 已落地；真实第二家 Sandbox 仍未实现。
...
BYD 提审协议无外部幂等键和状态查询，因此不套用下表的通用“网络/
...
| `UNKNOWN` | 未分类错误 | 按连接器是否具备幂等/查询能力决策；BYD 提审直接人工，不自动重发 |
...
BYD 提审重发只有在严格 ACK 落账后才发布恢复事件；发送成功、超时或本地猜测均不能触发
异常闭环。

## 13. 人工修复与重放

人工处理不能直接把 delivery 状态改为技术成功。单笔重发、远端查询、人工确认/放弃已落地：

- 修复配置后重试原 payload；
- 查询外部状态并登记观察；
- 标记外部已人工处理并附证据 / 放弃交付（状态保持 UNKNOWN + disposition）；
- 基于新对象版本创建新的 delivery 仍未实现；批量 ReplayRequest 已落地。
