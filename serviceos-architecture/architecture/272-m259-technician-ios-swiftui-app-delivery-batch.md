...
在 `/Users/louis/Downloads/Xcode-beta.app`（Xcode 27.0，iOS 27 SDK）中通过共享 scheme 已证明：
...
Production `.xcconfig` 已要求显式注入环境地址，但 Production 优化构建尚未在本批次独立执行；不得把
Test 配置的构建结果外推为发布候选证据。
