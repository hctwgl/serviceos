...
export async function loadTechnicianSession(preferredContextId?: string): Promise<TechnicianSession> {
  const contextsResult = await apiGet<ContextsResponse>('/me/contexts')
  const contexts = contextsResult.contexts.filter((item) => item.portal === 'TECHNICIAN')
  const first = contexts[0]
  if (!first) throw new Error('当前主体没有可用的 TECHNICIAN 上下文')
  if (preferredContextId && !contexts.some((item) => item.contextId === preferredContextId)) {
    throw new Error('TECHNICIAN 上下文不属于当前主体')
  }
  ...
  if (capabilities.portal !== 'TECHNICIAN' || navigation.portal !== 'TECHNICIAN') {
