...
test.describe('M389 工单详情统一履约工作区', () => {
  test('展示履约进度、审核记录与资料预览', async ({ page }) => {
    ...
    await expect(page.getByText('风险与提醒')).toBeVisible()
    await expect(page.getByText('当前责任链')).toBeVisible()
    ...
    await expect(page.getByText('技术诊断')).toHaveCount(0)
    await expect(page.getByText('PILOT_RESPONSE')).toHaveCount(0)
    await expect(page.getByText('assign-candidates')).toHaveCount(0)
    await expect(page.getByText('principalIds')).toHaveCount(0)
