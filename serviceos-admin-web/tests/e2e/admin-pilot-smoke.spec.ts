...
test('真实 OIDC 登录后可读取核心投影并完成 Task 分配领取释放写链路', async ({ page }) => {
  ...
  const [workOrderFilterResponse] = await Promise.all([
    ...
    clickWorkOrderQuery(page),
  ])
  expect(workOrderFilterResponse.status()).toBe(200)
  ...
  await fillWorkOrderProjectFilter(page, pilotProjectId)
  const [pilotDirectoryResponse] = await Promise.all([
    page.waitForResponse(
      (response) =>
        response.request().method() === 'GET' &&
        new URL(response.url()).pathname === '/api/v1/work-orders' &&
  ...
  expect(pilotDirectoryResponse.status()).toBe(200)
