...
test('真实 OIDC 登录后可读取核心投影并完成 Task 分配领取释放写链路', async ({ page }) => {
  ...
  await loginWithLocalKeycloak(page)
  await expect(page.getByRole('heading', { name: /授权工单目录|工单中心/ })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: /授权任务目录|任务中心|任务目录/ })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: /授权项目目录|项目中心|项目目录/ })).toBeVisible()
...
test('真实 OIDC 登录后可通过审核外发并经厂端回调关闭 CLIENT Case', async ({ page, request }) => {
  ...
  await expect(reviewPage.getByRole('heading', { name: /授权工单目录|工单中心/ })).toBeVisible()
