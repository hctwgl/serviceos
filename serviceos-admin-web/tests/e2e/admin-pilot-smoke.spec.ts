...
test('真实 OIDC 登录后可完成预约提议确认与上门签到签退', async ({ page }) => {
  ...
  await page.locator('summary', { hasText: '高级：一次分配网点与师傅（跳过网点接单）' }).click()
  ...
  await expect(page.getByRole('button', { name: '确认双责任分配' })).toBeVisible()
  ...
  await page.getByRole('button', { name: '确认双责任分配' }).click()
...
test('真实 OIDC 登录后可通过审核外发并经厂端回调关闭 CLIENT Case', async ({ page, request }) => {
  ...
  await reviewPage.getByRole('tab', { name: '集成' }).click()
...
test('真实 OIDC 登录后可在入站工单上完成领取、预约上门、整改补传复审与外发', async ({
  ...
  await page.getByRole('tab', { name: '集成' }).click()
  await expect(page.getByText('区块加载中…')).toHaveCount(0)
  await expect(page.locator('.canonical-links').getByRole('link').first()).toBeVisible({ timeout: 30_000 })
