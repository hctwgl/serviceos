...
/**
 * 现场履约 E2E 通过正式责任链准备数据：平台先从权威候选中派网点，随后由该网点指派师傅。
 * 这里不能回退到历史双责任接口，否则测试会掩盖平台越过网点直接派师傅的产品越权。
 */
async function prepareServiceAssignmentThroughResponsibilityChain(
  ...
  const result = await page.evaluate(async (id) => {
    ...
    const candidateResponse = await fetch(
      `/api/v1/tasks/${id}/network-assignment-candidates`,
      { headers: commonHeaders },
    )
    const candidateText = await candidateResponse.text()
    if (!candidateResponse.ok) {
      return { stage: 'candidate', status: candidateResponse.status, body: candidateText }
    }
    const candidateView = JSON.parse(candidateText) as {
      businessType: string
      candidates: Array<{ networkId: string }>
    }
    const networkId = candidateView.candidates[0]?.networkId
    if (!networkId) {
      return { stage: 'candidate', status: 422, body: candidateText }
    }

    const networkResponse = await fetch(
      `/api/v1/tasks/${id}/service-assignments:manual-assign-network`,
      {
        method: 'POST',
        headers: {
          ...commonHeaders,
          'Idempotency-Key': `e2e-network-setup:${crypto.randomUUID()}`,
        },
        body: JSON.stringify({
          networkAssigneeId: networkId,
          businessType: candidateView.businessType,
        }),
      },
    )
    const networkText = await networkResponse.text()
    if (!networkResponse.ok) {
      return { stage: 'network', status: networkResponse.status, body: networkText }
    }

    const technicianResponse = await fetch(`/api/v1/network-portal/tasks/${id}:assign-technician`, {
      method: 'POST',
      headers: {
        ...commonHeaders,
        'X-Network-Context': `NETWORK|NETWORK|${networkId}`,
        'Idempotency-Key': `e2e-technician-setup:${crypto.randomUUID()}`,
      },
      body: JSON.stringify({
        technicianAssigneeId: 'd3500000-1000-4000-8000-000000000004',
        businessType: candidateView.businessType,
      }),
    })
    return {
      ...
      body: await technicianResponse.text(),
    }
  }, taskId)
  expect(result.status, `${result.stage}: ${result.body}`).toBe(200)
}
...
test('真实 OIDC 登录后可读取核心投影并完成 Task 分配领取释放写链路', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '项目详情', level: 1 })).toBeVisible()
  await page.getByRole('link', { name: /服务任务|任务目录/ }).click()
  ...
  await expect(page.getByRole('heading', { name: /工单工作区|工单详情/, level: 1 })).toBeVisible()
  // A+ 工单工作区首屏只展示业务对象、进度、责任、风险和产品化页签。
  await expect(page.getByTestId('work-order-summary-strip')).toBeVisible()
  ...
  await expect(page.getByText('当前责任链', { exact: true })).toBeVisible()
  await page.getByRole('tab', { name: '任务记录' }).click()
  await expect(page.getByText('试点勘测', { exact: true }).first()).toBeVisible()
  await page.getByRole('tab', { name: '操作日志' }).click()
  await expect(page.locator('.product-timeline').first()).toBeVisible()
  await expect(page.getByText('PILOT_RESPONSE', { exact: true })).toHaveCount(0)
  await expect(page.getByText('技术诊断', { exact: true })).toHaveCount(0)
  await expect(page.getByText('assign-candidates', { exact: true })).toHaveCount(0)
...
test('真实 OIDC 登录后可完成 Task 并可靠推进 Workflow 与 WorkOrder', async ({ page }) => {
  ...
  await clickTaskCommandButton(page, /完成任务|完成/)
...
test('真实 OIDC 登录后审核驳回可经补传关闭并复审通过后完结工单', async ({ page }) => {
  ...
  if (secondReviewCreate.status() === 201) {
    ...
    // 用新页签查队列，避免破坏源 Task 页的 prepared 双引用状态。
    const queuePage = await page.context().newPage()
    const reopenQueuePromise = queuePage.waitForResponse(
      (response) =>
        response.request().method() === 'GET' &&
        new URL(response.url()).pathname === '/api/v1/review-cases' &&
        new URL(response.url()).searchParams.get('status') === 'OPEN' &&
    ...
    await queuePage.goto(`/reviews?status=OPEN&taskId=${taskId}`)
...
test('真实 OIDC 登录后可完成预约提议确认与上门签到签退', async ({ page }) => {
  ...
  // M453：现场闭环的数据前置同样走“平台派网点 → 网点派师傅”的正式责任链。
  await prepareServiceAssignmentThroughResponsibilityChain(page, taskId!)
  // 产品化摘要用「服务网点/服务师傅」实体名呈现，不再暴露明文 assigneeId。
  await expect(page.getByText(/服务网点|网点/).first()).toBeVisible()

  await page.getByRole('tab', { name: '基本信息' }).click()
  await expect(page.getByLabel('assign-candidates principalIds')).toBeVisible({ timeout: 15_000 })
  await page
    .getByLabel('assign-candidates principalIds')
    .fill('06b612f3-a901-4b0e-bd90-86b4259cc087')
...
test('真实 OIDC 登录后可通过审核外发并经厂端回调关闭 CLIENT Case', async ({ page, request }) => {
  ...
  await reviewPage.goto(`/integration/outbound/${delivery.deliveryId}`)
  ...
  // M163：产品化后核心时间线侧链移除；轮询活动记录页签中的回执深链。
  ...
  await expect(reviewPage.getByRole('heading', { name: /工单工作区|工单详情/, level: 1 })).toBeVisible()
  await reviewPage.getByRole('tab', { name: '活动记录' }).click()
  const workspaceRefresh = reviewPage.getByRole('button', { name: '刷新' }).first()
  const receiptLink = reviewPage.getByRole('link', {
    name: /ExternalReviewReceipt|外部审核回执|evidence\.external-review-receipt-recorded/,
  })
  await expect
    .poll(
      async () => {
...
test('真实 OIDC 登录后可在入站工单上完成领取、预约上门、整改补传复审与外发', async ({
  ...
  // 冒烟脚本已完成 CPIM 入站与 Outbox 激活；此处经正式两阶段责任链后证明同单写路径
  ...
  await loginWithLocalKeycloak(page)
  // Ant Select 下拉在 headless 中不稳定；用 route.query 水合 status=ACTIVE。
  await page.goto('/work-orders?status=ACTIVE')
  ...
  await expect(page.getByRole('heading', { name: /工单工作区|工单详情/, level: 1 })).toBeVisible()
  await expect(page.getByText(/处理中|ACTIVE/).first()).toBeVisible()
  await expect(page.getByText(orderCode!).first()).toBeVisible()

  const integrationSectionPromise = page.waitForResponse(
    (response) =>
      response.request().method() === 'GET' &&
      new URL(response.url()).pathname.endsWith('/workspace/sections/INTEGRATION'),
  )
  await page.getByRole('tab', { name: '外部回传' }).click()
  expect((await integrationSectionPromise).status()).toBe(200)
  await expect(page.getByText('区块加载中…')).toHaveCount(0)
  // 产品化后不再渲染 sections JSON；用入站关联链接证明投影。
  await expect(page.getByText('打开入站 Envelope：')).toBeVisible({ timeout: 30_000 })
  await expect(page.locator('.inbound-links')).toContainText(/CREATE_WORK_ORDER/)
  await expect(page.locator('.inbound-links')).toContainText(/COMPLETED|已完成/)
  ...
  // 整改关闭会在同一事务内为最新补传快照自动创建复审案例；从权威审核队列进入，
  // 不能再手工重复创建同一 Snapshot 的 ReviewCase。审核队列使用独立页面，保留源任务页
  // 已准备好的表单与资料双引用，后续仍由正式 Task complete 命令提交。
  const secondReviewQueuePage = await page.context().newPage()
  const secondReviewQueuePromise = secondReviewQueuePage.waitForResponse(
    (response) =>
      response.request().method() === 'GET' &&
      new URL(response.url()).pathname === '/api/v1/review-cases' &&
      new URL(response.url()).searchParams.get('status') === 'OPEN' &&
  ...
  await secondReviewQueuePage.goto(`/reviews?status=OPEN&taskId=${taskId}`)
  const secondReviewQueueResponse = await secondReviewQueuePromise
  expect(secondReviewQueueResponse.status()).toBe(200)
  const secondReviewQueue = (await secondReviewQueueResponse.json()) as {
    items: Array<{
      reviewCaseId: string
      status: string
      evidenceSetSnapshotId: string
  ...
  const approvedReview = secondReviewQueue.items.find(
    (item) => item.evidenceSetSnapshotId === secondSnapshot.evidenceSetSnapshotId,
  )
  expect(approvedReview, '整改关闭后未自动创建补传快照复审案例').toBeTruthy()
  expect(approvedReview).toMatchObject({
    status: 'OPEN',
    evidenceSetSnapshotId: secondSnapshot.evidenceSetSnapshotId,
  })
  ...
  const reReviewHref = await secondReviewQueuePage
  ...
  await reReviewPage.close()
  await secondReviewQueuePage.close()
  await correctionPage.close()
  await reviewPage.close()
})
