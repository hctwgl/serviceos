 ...
 * 不能再用裸 taskId 正则，否则 strict mode 会命中两条同目标链接。
 */
...
test('真实 OIDC 登录后可读取核心投影并完成 Task 分配领取释放写链路', async ({ page }) => {
  ...
  await page
    .getByRole('link', {
      name: new RegExp(`PILOT_RESPONSE\\s*/\\s*${pilotTaskId}`),
    })
    .click()
  ...
  await page
    .getByRole('link', {
      name: new RegExp(`PILOT_RESPONSE\\s*/\\s*${taskId}`),
    })
    .click()
...
test('真实 OIDC 登录后可在入站工单上完成领取、预约上门、整改补传复审与外发', async ({
  ...
  await page.getByRole('link', { name: /CREATE_WORK_ORDER\s*\/\s*COMPLETED/ }).click()
