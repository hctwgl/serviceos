...
test('真实 OIDC 登录后可读取核心投影并完成 Task 分配领取释放写链路', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '任务中心', level: 1 })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: /项目列表|项目中心|授权项目目录|项目目录/ })).toBeVisible()
  ...
  await page
    ...
    .getByRole('link', {
      name: new RegExp(`activity\\s*/\\s*[^/]+\\s*/\\s*Task\\s*/\\s*(试点勘测|PILOT_SURVEY|70000000-0000-4000-8000-000000000001)`),
    })
    .click()
  ...
  await page
    .getByRole('link', {
      name: new RegExp(`试点勘测\\s*/\\s*人工任务\\s*/\\s*待开始\\s*/\\s*${taskId}`),
    })
    .click()
  ...
  await page.getByRole('button', { name: /TIMELINE_AUDIT|时间线/ }).click()
  await expect(page.getByText('区块加载中…')).toHaveCount(0)
...
test('真实 OIDC 登录后审核驳回可进入整改队列并授权豁免整改 Task', async ({ page }) => {
  ...
  await page
    .getByRole('link', {
      name: new RegExp(
        `(INTERNAL|内部)\\s*/\\s*(REJECTED|已驳回)\\s*/\\s*${reviewCase.reviewCaseId}`,
      ),
    })
    .click()
  ...
  await page
    .getByRole('link', {
      name: new RegExp(
        `(?:IN_PROGRESS|处理中)\\s*/\\s*${workspaceCorrection!.correctionCaseId}`,
      ),
    })
    .click()
  ...
  // M179：整改详情明文 projectId → 项目详情深链（承接 M172 未覆盖的详情页）。
  const correctionProjectPromise = page.waitForResponse(
    (response) =>
      response.request().method() === 'GET' &&
      new URL(response.url()).pathname === `/api/v1/projects/${pilotProjectId}`,
  )
  await page
    ...
    .getByRole('link', { name: new RegExp(`打开项目\\s+${pilotProjectId}`) })
    .click()
  expect((await correctionProjectPromise).status()).toBe(200)
  await expect(page.getByRole('heading', { name: '项目详情' })).toBeVisible()
  await expect(page).toHaveURL(new RegExp(`/projects/${pilotProjectId}$`))
  await page.goto(
    new URL(`/corrections/${workspaceCorrection!.correctionCaseId}`, page.url()).toString(),
  )
  await expect(page.getByRole('heading', { name: '整改案例' })).toBeVisible()
...
test('真实 OIDC 登录后可完成预约提议确认与上门签到签退', async ({ page }) => {
  ...
  await page.getByText('高级：一次初派网点+师傅（跳过网点接单）').click()
  ...
  await expect(page.getByText(/已初派：网点\s+d3500000-1000-4000-8000-000000000002/)).toBeVisible()
  await expect(
    page.locator('dt', { hasText: /^服务责任$/ }).locator('xpath=../dd'),
  ).toContainText('d3500000-1000-4000-8000-000000000002')
...
test('真实 OIDC 登录后可在入站工单上完成领取、预约上门、整改补传复审与外发', async ({
  ...
  await page.getByLabel('workOrder status filter').selectOption('ACTIVE')
  await page.getByRole('button', { name: '查询' }).click()
  ...
  await expect(page.getByRole('heading', { name: '工单工作区' })).toBeVisible()
  await expect(page.locator('dt', { hasText: /^状态$/ }).locator('xpath=../dd')).toHaveText(/ACTIVE|处理中/)
  await expect(
  ...
  await expect(page.getByText(/已初派：网点/)).toBeVisible()
