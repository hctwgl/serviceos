...
test.describe('M235 Network Portal 目录页资料 Evidence 服务端摘要', () => {
  test('M235-05a：工单目录展示资料列', async ({ page }) => {
    ...
    await expect(page.getByTestId('work-order-evidence-summary')).toContainText('MISSING')
    await expect(page.getByTestId('work-order-evidence-summary')).toContainText('OPEN项')
  })
  ...
  test('M235-05c：任务目录展示资料列', async ({ page }) => {
    ...
    await expect(page.getByTestId('task-evidence-summary')).toContainText('MISSING')
  })
})
