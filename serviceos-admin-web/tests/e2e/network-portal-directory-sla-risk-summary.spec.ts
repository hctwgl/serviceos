...
test.describe('M234 Network Portal 目录页 SLA 风险服务端摘要', () => {
  test('M234-05a：工单目录展示 SLA 风险列', async ({ page }) => {
    ...
    await expect(page.getByTestId('work-order-sla-risk')).toContainText('开放 2')
    await expect(page.getByTestId('work-order-sla-risk')).toContainText('超时 1')
  })

  test('M234-05b：缺 slaRiskSummaries 时省略 SLA 风险列', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubContexts(page)
    await page.route('**/api/v1/network-portal/work-orders**', async (route: Route) => {
      if (route.request().method() !== 'GET') {
    ...
    await page.goto('/network-portal/work-orders')
    await expect(page.getByTestId('network-work-orders-table')).toBeVisible({ timeout: 15_000 })
    await expect(page.getByTestId('work-order-sla-risk')).toHaveCount(0)
  })

  test('M234-05c：任务目录展示 SLA 风险列', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubContexts(page)
    ...
    await expect(page.getByTestId('task-sla-risk')).toContainText('超时 1')
  })
})
