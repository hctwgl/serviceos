...
test.describe('M232 Network Portal 目录页联系尝试服务端摘要', () => {
  test('M232-05a：工单目录展示最近联系列', async ({ page }) => {
    ...
    await page.route('**/api/v1/network-portal/work-orders**', async (route: Route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          contactAttempts: [contactSummary],
        }),
      })
    })
    ...
    await expect(page.getByTestId('work-order-contact-attempt')).toContainText('PHONE')
    await expect(page.getByTestId('work-order-contact-attempt')).toContainText('CONNECTED')
  })

  test('M232-05b：缺 contactAttempts 时省略最近联系列', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubContexts(page)
    await page.route('**/api/v1/network-portal/work-orders**', async (route: Route) => {
      if (route.request().method() !== 'GET') {
    ...
    await page.goto('/network-portal/work-orders')
    await expect(page.getByTestId('network-work-orders-table')).toBeVisible({ timeout: 15_000 })
    await expect(page.getByTestId('work-order-contact-attempt')).toHaveCount(0)
  })

  test('M232-05c：任务目录展示最近联系列', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubContexts(page)
    ...
    await expect(page.getByTestId('task-contact-attempt')).toContainText('CONNECTED')
  })
})
