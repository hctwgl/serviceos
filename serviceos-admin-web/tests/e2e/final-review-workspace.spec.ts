...
const REVIEW_CASE_ID = '22222222-2222-4222-8222-222222222222'
...
function fakeAccessToken(sub = 'e2e-reviewer') {
...
function finalReviewFixture(overrides: Record<string, unknown> = {}) {
  const base = {
    data: {
      workOrder: {
...
async function mockFinalReviewApis(page: Page, fixture = finalReviewFixture()) {
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.includes('/me/ui-preferences') || path.includes('/ui-preferences')) {
      await fulfillJson(route, { key: 'admin', value: {}, schemaVersion: 1, aggregateVersion: 1, updatedAt: '2026-07-19T03:00:00Z' })
      return
    }
    ...
    if (path.includes(':decide') && method === 'POST') {
      await fulfillJson(route, {
        reviewCaseId: REVIEW_CASE_ID,
        projectId: fixture.data.workOrder.projectId,
        taskId: fixture.data.reviewTask.taskId,
