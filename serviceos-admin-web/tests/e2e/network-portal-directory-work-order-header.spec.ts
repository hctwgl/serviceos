...
test.describe('M236 Network Portal 目录页工单头字段', () => {
  test('M236-05a：工单目录展示服务产品/区域/接收时间', async ({ page }) => {
          ...
          items: [
            {
              ...
              cityCode: '370100',
              districtCode: '370102',
              receivedAt: '2026-07-17T02:00:00Z',
            },
          ],
        }),
      })
    ...
    await expect(page.getByTestId('work-order-service-product')).toContainText('HOME_CHARGING')
    await expect(page.getByTestId('work-order-region')).toContainText('370000')
    await expect(page.getByTestId('work-order-received-at')).toContainText('2026-07-17')
  })

  test('M236-05b：任务目录展示服务产品/区域', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubContexts(page)
    ...
    await expect(page.getByTestId('task-service-product')).toContainText('HOME_CHARGING')
    await expect(page.getByTestId('task-region')).toContainText('370100')
  })
})
