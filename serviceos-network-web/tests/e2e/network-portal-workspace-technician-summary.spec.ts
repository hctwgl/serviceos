...
async function stubWorkspaceTechnicians(
  page: Page,
  options?: { includeTechnicians?: boolean },
) {
  const includeTechnicians = options?.includeTechnicians !== false
...
test.describe('M228 Network Portal 工作区当前师傅服务端摘要', () => {
  test('M228-05a：展示 technicians 摘要与深链', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubWorkspaceTechnicians(page)
  ...
  test('M228-05b：缺字段时省略区块（不得伪装为空列表）', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubWorkspaceTechnicians(page, { includeTechnicians: false })
    await navigateNetwork(page, `/network-portal/work-orders/${WORK_ORDER_ID}`)
    await expect(page.getByTestId('network-portal-work-order-workspace')).toBeVisible({
      timeout: 15_000,
    })
