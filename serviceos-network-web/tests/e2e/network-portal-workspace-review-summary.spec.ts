...
const REVIEW_ID = '019f84a0-dddd-7f8c-9505-36fe5c0ee004'
...
async function stubWorkspaceReviews(
  page: Page,
        ...
        body.reviews = [
          {
            ...
            evidenceSetSnapshotId: '019f84a0-ffff-7f8c-9505-36fe5c0ee006',
            scopeType: 'EVIDENCE_SET_SNAPSHOT',
            origin: 'INTERNAL',
            policyVersion: 'POLICY_V1',
            status: 'REJECTED',
            createdAt: '2026-07-17T11:00:00Z',
            decidedAt: '2026-07-17T11:05:00Z',
            ...
            decisions: [
              {
                reviewDecisionId: '019f84a0-1111-7f8c-9505-36fe5c0ee007',
                decisionOrdinal: 1,
                decision: 'REJECTED',
...
test.describe('M229 Network Portal 工作区审核摘要', () => {
  test('M229-05a：展示 reviews 摘要与任务深链', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubWorkspaceReviews(page)
    ...
    await expect(page.getByTestId('workspace-related-reviews')).toBeVisible()
    await expect(page.getByTestId(`workspace-related-review-${REVIEW_ID}`)).toContainText(
      'REJECTED',
    )
    await expect(
      page.getByTestId(`workspace-related-review-${REVIEW_ID}`).locator('a'),
    ).toHaveAttribute('href', `/network-portal/tasks?taskId=${TASK_ID}`)
  })

  test('M229-05b：缺字段时省略区块（不得伪装为空列表）', async ({ page }) => {
    await loginWithLocalKeycloak(page)
    await stubWorkspaceReviews(page, { includeReviews: false })
    await navigateNetwork(page, `/network-portal/work-orders/${WORK_ORDER_ID}`)
    await expect(page.getByTestId('network-portal-work-order-workspace')).toBeVisible({
      timeout: 15_000,
    })
    await expect(page.getByTestId('workspace-header-fields')).toBeVisible()
    await expect(page.getByTestId('workspace-related-reviews')).toHaveCount(0)
  })
})
