...
test.describe('M202 Network Portal 整改队列', () => {
  test('M202-08：伪造上下文拒绝；有 NETWORK 上下文时进入整改页', async ({ page }) => {
    ...
    const correctionsNav = page.getByTestId('nav-network-corrections')
    if (await correctionsNav.count()) {
      await correctionsNav.click()
      await expect(page.getByTestId('network-portal-corrections')).toBeVisible()
      await expect(page.locator('[data-page-id="NETWORK.CORRECTION.QUEUE"]')).toBeVisible()
    } else {
