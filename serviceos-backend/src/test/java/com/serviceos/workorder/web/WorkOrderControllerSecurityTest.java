...
class WorkOrderControllerSecurityTest {
 ...
 @Test void trustedPrincipalDrivesListAndDetailWithEtag() throws Exception {
  ...
  mvc.perform(get("/api/v1/work-orders").with(jwt()).header("X-Correlation-Id","corr-list").queryParam("clientCode","BYD").queryParam("status","RECEIVED").queryParam("limit","20"))
    .andExpect(status().isOk()).andExpect(header().string("X-Correlation-Id","corr-list")).andExpect(jsonPath("$.items[0].id").value(id.toString())).andExpect(jsonPath("$.items[0].maskedCustomerPhone").value("*******0000")).andExpect(jsonPath("$.items[0].updatedAt").value("2026-07-15T03:30:00Z")).andExpect(jsonPath("$.items[0].currentStageCode").value("SURVEY")).andExpect(jsonPath("$.items[0].currentClaimedBy").value(claimant)).andExpect(jsonPath("$.items[0].currentAssigneeDisplayName").value("演示师傅")).andExpect(jsonPath("$.items[0].customerMobile").doesNotExist());
