...
class WorkOrderControllerSecurityTest {
 ...
 @Test void trustedPrincipalDrivesListAndDetailWithEtag() throws Exception {
  ...
  WorkOrderView view=new WorkOrderView(id,"tenant-trusted",project,"BYD","OCEAN","INSTALL","EXT-1","RECEIVED",bundle,"B","1.0.0","a".repeat(64),"370000","370100","370102",Instant.parse("2026-07-15T02:00:00Z"),Instant.parse("2026-07-15T03:00:00Z"),null,null,3,"王*","*******0000","杭州市***");
  ...
  mvc.perform(get("/api/v1/work-orders").with(jwt()).header("X-Correlation-Id","corr-list").queryParam("clientCode","BYD").queryParam("status","RECEIVED").queryParam("limit","20"))
    .andExpect(status().isOk()).andExpect(header().string("X-Correlation-Id","corr-list")).andExpect(jsonPath("$.items[0].id").value(id.toString())).andExpect(jsonPath("$.items[0].maskedCustomerPhone").value("*******0000")).andExpect(jsonPath("$.items[0].customerMobile").doesNotExist());
  mvc.perform(get("/api/v1/work-orders/{id}",id).with(jwt()).header("X-Correlation-Id","corr-get"))
    .andExpect(status().isOk()).andExpect(header().string("ETag","\"3\"")).andExpect(jsonPath("$.workOrder.id").value(id.toString())).andExpect(jsonPath("$.workOrder.maskedCustomerName").value("王*"));
