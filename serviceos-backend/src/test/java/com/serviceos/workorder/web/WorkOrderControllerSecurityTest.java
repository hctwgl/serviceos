...
class WorkOrderControllerSecurityTest {
 ...
 @Test void trustedPrincipalDrivesListAndDetailWithEtag() throws Exception {
  ...
  mvc.perform(get("/api/v1/work-orders").with(jwt()).header("X-Correlation-Id","corr-list").queryParam("clientCode","BYD").queryParam("status","RECEIVED").queryParam("districtCode","370102").queryParam("currentStageCode","SURVEY").queryParam("currentNetworkId","bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb").queryParam("currentTechnicianId","cccccccc-cccc-4ccc-8ccc-cccccccccccc").queryParam("slaRisk","OPEN").queryParam("receivedFrom","2026-03-01").queryParam("receivedTo","2026-03-31").queryParam("limit","20"))
    .andExpect(status().isOk()).andExpect(header().string("X-Correlation-Id","corr-list")).andExpect(jsonPath("$.items[0].id").value(id.toString())).andExpect(jsonPath("$.items[0].maskedCustomerPhone").value("*******0000")).andExpect(jsonPath("$.items[0].updatedAt").value("2026-07-15T03:30:00Z")).andExpect(jsonPath("$.items[0].currentStageCode").value("SURVEY")).andExpect(jsonPath("$.items[0].currentClaimedBy").value(claimant)).andExpect(jsonPath("$.items[0].currentAssigneeDisplayName").value("演示师傅")).andExpect(jsonPath("$.items[0].currentNetworkDisplayName").value("青岛网点")).andExpect(jsonPath("$.items[0].currentTechnicianDisplayName").value("现场师傅")).andExpect(jsonPath("$.totalCount").value(1)).andExpect(jsonPath("$.totalCountTruncated").value(false)).andExpect(jsonPath("$.items[0].customerMobile").doesNotExist());
