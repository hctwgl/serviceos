...
class WorkOrderControllerSecurityTest {
 ...
 @Test void trustedPrincipalDrivesListAndDetailWithEtag() throws Exception {
  ...
  mvc.perform(get("/api/v1/work-orders").with(jwt()).header("X-Correlation-Id","corr-list").queryParam("clientCode","BYD").queryParam("status","RECEIVED").queryParam("districtCode","370102").queryParam("currentStageCode","SURVEY").queryParam("currentNetworkId","bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb").queryParam("limit","20"))
  ...
  verify(queries).list(eq(principal),eq("corr-list"),argThat(q->"BYD".equals(q.clientCode())&&"RECEIVED".equals(q.status())&&"370102".equals(q.districtCode())&&"SURVEY".equals(q.currentStageCode())&&UUID.fromString("bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb").equals(q.currentNetworkId())&&q.limit()==20));
 }
}
