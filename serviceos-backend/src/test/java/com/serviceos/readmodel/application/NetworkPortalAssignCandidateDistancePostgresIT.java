...
/**
 * M410：分配候选行政区距离亲和读模型（Coverage + 工单省市区；无伪造经纬度）。
 */
@Testcontainers(disabledWithoutDocker = true)
@SpringBootTest(classes = ServiceOsApplication.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
class NetworkPortalAssignCandidateDistancePostgresIT {
    private static final String TENANT = "tenant-network-portal-m410";
    private static final UUID PRINCIPAL = UUID.fromString("019f9100-4110-7000-8000-000000000001");
    private static final UUID NETWORK = UUID.fromString("019f9100-4110-7000-8000-000000000010");
    private static final UUID PARTNER = UUID.fromString("019f9100-4110-7000-8000-000000000020");
    private static final UUID TECH_PROFILE = UUID.fromString("019f9100-4110-7000-8000-000000000030");
    private static final UUID TECH_PRINCIPAL = UUID.fromString("019f9100-4110-7000-8000-000000000031");
    private static final UUID WO = UUID.fromString("019f9100-4110-7000-8000-000000000040");
    private static final UUID TASK = UUID.fromString("019f9100-4110-7000-8000-000000000050");
    ...
    void assignCandidatesExposeRegionDistanceAffinity() {
        ...
        assertThat(tech.distanceSummary()).contains("同城").contains("青岛市");
        assertThat(tech.assignable()).isTrue();
    }

    @Test
    void outsideCoverageWhenRegionMisses() {
        jdbc.sql("DELETE FROM net_service_network_coverage").update();
        seedCoverage(NETWORK, "BYD_OCEAN", "HOME_CHARGING", "370100");

        NetworkPortalAssignCandidatePage page = portal.listAssignCandidates(
                actor(PRINCIPAL), "corr-m410-out", "NETWORK|NETWORK|" + NETWORK, TASK);
        NetworkPortalAssignCandidateItem tech = page.items().getFirst();
        assertThat(tech.distanceTier()).isEqualTo("OUTSIDE_COVERAGE");
        assertThat(tech.coverageMatched()).isFalse();
        assertThat(tech.distanceSummary()).contains("覆盖未命中");
        assertThat(tech.warnings()).anyMatch(w -> w.contains("覆盖未命中"));
    }
    ...
    private void seedWorkOrderHeader(
        ...
        jdbc.sql("""
                ...
                VALUES (
                    :projectId, :tenantId, :code, 'BYD', 'M410 distance fixture',
                    DATE '2026-07-01', NULL, 'ACTIVE', 1, :createdAt)
                ON CONFLICT (project_id) DO NOTHING
                """)
                .param("projectId", projectId)
                .param("tenantId", TENANT)
                .param("code", "M410-" + workOrderId.toString().substring(24))
        ...
        jdbc.sql("""
                ...
                .param("tenantId", TENANT)
                .param("projectId", projectId)
                .param("bundleCode", "M410-BUNDLE-" + workOrderId.toString().substring(24))
        ...
        jdbc.sql("""
                INSERT INTO wo_work_order (
                    ...
                    :bundleId, 'M410-BUNDLE', '1.0.0', :bundleDigest,
                ...
                .param("brandCode", brandCode)
                .param("product", serviceProductCode)
                .param("externalOrderCode", "M410-" + workOrderId)
    ...
    private void seedGrant(UUID principalId, String capability, String scopeType, String scopeRef) {
        ...
        jdbc.sql("""
                ...
                .param("roleId", roleId)
                .param("tenant", TENANT)
                .param("code", "m410-" + capability + "-" + UUID.randomUUID())
                .update();
        jdbc.sql("""
