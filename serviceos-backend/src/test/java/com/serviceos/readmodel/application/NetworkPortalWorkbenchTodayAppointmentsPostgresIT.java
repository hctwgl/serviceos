...
/**
 * M411：工作台今日预约时间轴（Asia/Shanghai 运营日 + manageAppointment soft-gate）。
 */
@Testcontainers(disabledWithoutDocker = true)
@SpringBootTest(classes = ServiceOsApplication.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
class NetworkPortalWorkbenchTodayAppointmentsPostgresIT {
    private static final String TENANT = "tenant-network-portal-m411";
    private static final UUID PRINCIPAL = UUID.fromString("019f9111-4111-7000-8000-000000000001");
    private static final UUID NETWORK = UUID.fromString("019f9111-4111-7000-8000-000000000010");
    private static final UUID PARTNER = UUID.fromString("019f9111-4111-7000-8000-000000000020");
    private static final UUID TECH_PROFILE = UUID.fromString("019f9111-4111-7000-8000-000000000030");
    private static final UUID TECH_PRINCIPAL = UUID.fromString("019f9111-4111-7000-8000-000000000031");
    private static final UUID WO = UUID.fromString("019f9111-4111-7000-8000-000000000040");
    private static final UUID TASK = UUID.fromString("019f9111-4111-7000-8000-000000000050");
    private static final UUID APPOINTMENT = UUID.fromString("019f9111-4111-7000-8000-000000000060");
    private static final UUID REVISION = UUID.fromString("019f9111-4111-7000-8000-000000000061");
    private static final UUID PROJECT = UUID.fromString("019f9111-4111-7000-8000-000000000070");
    ...
    @Test
    void timelineAlwaysPresentAndAppointmentsSoftGated() {
        NetworkPortalWorkbenchView base = portal.workbench(
                actor(PRINCIPAL), "corr-m411-base", "NETWORK|NETWORK|" + NETWORK);
        assertThat(base.todayTimeline())
                .extracting(NetworkPortalWorkbenchTimelineBucket::bucketCode)
                .contains(NetworkPortalWorkbenchTimelineBucket.UNASSIGNED);
        assertThat(base.todayAppointmentCount()).isNull();
        assertThat(base.todayAppointments()).isNull();

        seedGrant(PRINCIPAL, "networkPortal.manageAppointment", "NETWORK", NETWORK.toString());
        jdbc.sql("""
                UPDATE auth_tenant_grant_generation SET generation = generation + 1, updated_at = now()
                 WHERE tenant_id = :tenant
                """).param("tenant", TENANT).update();

        NetworkPortalWorkbenchView withApt = portal.workbench(
                actor(PRINCIPAL), "corr-m411-apt", "NETWORK|NETWORK|" + NETWORK);
        assertThat(withApt.todayAppointmentCount()).isEqualTo(1);
        assertThat(withApt.todayAppointments()).hasSize(1);
        assertThat(withApt.todayAppointments().getFirst().appointmentId()).isEqualTo(APPOINTMENT);
        assertThat(withApt.todayAppointments().getFirst().technicianDisplayName()).isEqualTo("青岛师傅");
        assertThat(withApt.todayTimeline())
                .extracting(NetworkPortalWorkbenchTimelineBucket::bucketCode)
                .contains(
        ...
        int appointmentBuckets = withApt.todayTimeline().stream()
                .filter(b -> b.bucketCode().endsWith("_APPOINTMENTS"))
                .mapToInt(NetworkPortalWorkbenchTimelineBucket::count)
                .sum();
        assertThat(appointmentBuckets).isEqualTo(1);
    }
    ...
    private void seedHumanTask(UUID taskId, UUID workOrderId) {
        ...
        jdbc.sql("""
                ...
                VALUES (
                    :projectId, :tenantId, 'M411-P', 'BYD', 'M411 today appointment',
    ...
    private void seedGrant(UUID principalId, String capability, String scopeType, String scopeRef) {
        ...
        jdbc.sql("""
                ...
                .param("roleId", roleId)
                .param("tenant", TENANT)
                .param("code", "m411-" + capability + "-" + UUID.randomUUID())
                .update();
        jdbc.sql("""
