...
class ConfigurationClientCapabilityGateTest {
    ...
    @Test
    void webOnlyVisibleWhenPublishesWithoutIosReport() {
        ClientCompatibilityReport report = gate.evaluate(
                ConfigurationAssetType.FORM, """
                        {"formKey":"cond","version":"1.0.0","stage":"SURVEY","sections":[{"sectionKey":"s",
                         ...
                         "visibleWhen":{"language":"SERVICEOS_EXPR_V1","source":"formValues[\\"need\\"] == true"}}]}]}
                        """,
                java.util.List.of("TECHNICIAN_WEB"));
        assertThat(report.blocking()).isFalse();
        assertThat(report.clientReports()).hasSize(1);
        assertThat(report.clientReports().getFirst().clientKind()).isEqualTo("TECHNICIAN_WEB");
        assertThat(report.clientReports().getFirst().compatible()).isTrue();
    }

    @Test
    void directedBothKindsHardBlocksVisibleWhenOnIosGap() {
        ClientCompatibilityReport report = gate.evaluate(
                ConfigurationAssetType.FORM, """
                        {"formKey":"cond","version":"1.0.0","stage":"SURVEY","sections":[{"sectionKey":"s",
                         "title":"S","fields":[{"fieldKey":"need","label":"需要","dataType":"BOOLEAN",
                         ...
                         "visibleWhen":{"language":"SERVICEOS_EXPR_V1","source":"formValues[\\"need\\"] == true"}}]}]}
                        """,
                java.util.List.of("TECHNICIAN_WEB", "TECHNICIAN_IOS"));
        assertThat(report.blocking()).isTrue();
        assertThat(report.blockingErrors().getFirst()).contains("TECHNICIAN_IOS");
        assertThat(report.blockingErrors().getFirst()).contains("form.condition.visibleWhen");
    }
}
