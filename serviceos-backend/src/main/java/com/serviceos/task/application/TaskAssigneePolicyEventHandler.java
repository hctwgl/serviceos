...
/** 消费 task.created，将冻结 ASSIGNEE_POLICY 解析为候选快照。 */
@Service
final class TaskAssigneePolicyEventHandler implements OutboxMessageHandler {
    private final TaskAssigneePolicyEventConsumer consumer;
    private final ObjectMapper objectMapper;

    TaskAssigneePolicyEventHandler(TaskAssigneePolicyEventConsumer consumer, ObjectMapper objectMapper) {
    ...
    public void handle(OutboxMessage message) {
        if (!"task".equals(message.module()) || !"Task".equals(message.aggregateType())) {
            throw new IllegalArgumentException("unsupported Task assignee-policy event envelope");
        }
        ...
        if (payload.taskId() == null || payload.createdAt() == null
            ...
            throw new IllegalArgumentException("Task assignee-policy event identity mismatch");
        }
    ...
    private <T> T read(String payload, Class<T> type) {
        try {
            ...
            throw new IllegalArgumentException("Task assignee-policy event payload cannot be decoded", exception);
        }
    }
