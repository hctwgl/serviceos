...
/** 现场到场、离场和中断的 HTTP 适配器；身份与租户只能来自受信 JWT 主体。 */
@RestController
...
final class VisitController {
    ...
    VisitController(VisitService visits, CurrentPrincipalProvider principals) {
        this.visits = visits;
        this.principals = principals;
    }

    @GetMapping("/work-orders/{workOrderId}/visits")
    List<VisitView> list(
    ...
    ResponseEntity<VisitCommandReceipt> checkIn(
        ...
        VisitCommandReceipt receipt = visits.checkIn(
                ...
                new CheckInVisitCommand(appointmentId, request.capturedAt(), request.deviceCommandId(),
                        request.deviceId(), request.location().toLocation(), request.offline()));
    ...
    private static ResponseEntity<VisitCommandReceipt> response(
            VisitCommandReceipt receipt, String correlationId
    ) {
    ...
    record LocationRequest(
        ...
        VisitLocation toLocation() { return new VisitLocation(latitude, longitude, accuracyMeters); }
    }
    ...
    record CheckOutRequest(
            @NotNull Instant capturedAt,
            @NotBlank @Size(max = 100) String resultCode,
    ...
    record InterruptRequest(
            @NotNull Instant capturedAt,
            @NotBlank @Size(max = 100) String exceptionCode,
