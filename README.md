...
当前工程基线推进至 **M274**：在 M267 通用 Connector SPI / BYD 入站归位之上，已交付 Workflow
`transition.condition` 与 FORM/EVIDENCE 对齐的表达式对象、EXCLUSIVE_GATEWAY 发布期静态校验与 schema
漂移门禁；网关运行时仍未实现。Track A、独立 Network/Technician Web、iOS Simulator/分发基础与在线
Visit/表单/Evidence/Snapshot/整改仍有效。本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级
表单与草稿、真实 operationRef 签退、弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight
尚未验收。下一主线进入阶段二复杂流程（PARALLEL_GATEWAY 等）与领域配置设计器。
