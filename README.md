...
当前工程基线推进至 **M85**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M85 077 79
```
...
[M85 工单工作区只读组合快照](serviceos-architecture/architecture/98-m85-work-order-workspace-query.md)。
