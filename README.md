...
当前工程基线推进至 **M323**：`task.created` 上冻结 ASSIGNEE_POLICY 自动写入 TaskAssignment CANDIDATE
（空池/MANUAL 失败关闭）。M321/M322 已交付入站 Mapping 物化与出站 Mapping 提审 Payload。
...
下一主线继续 **Configuration-Driven Fulfillment Runtime**：DISPATCH→ServiceAssignment、RULE 门禁、
通知投递闭环与履约试算快照；吉利真实 Sandbox/凭据与 Track F/G 仍为 `BLOCKED_EXTERNAL`，
