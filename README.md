...
当前工程基线推进至 **M87**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M87 078 80
```
...
[M87 工单工作区按需区块加载](serviceos-architecture/architecture/100-m87-work-order-workspace-sections.md)。
