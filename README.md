...
当前工程基线推进至 **M90**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M90 078 80
```
...
[M90 工单工作区审核与整改区块](serviceos-architecture/architecture/103-m90-work-order-workspace-reviews-corrections.md)。
