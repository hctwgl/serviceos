...
当前工程基线推进至 **M350 Technician 表达式上下文与 validationRules**（基线含 M349（`c54c98ee`）；M350 Draft 于本切片）。
M348 DISPATCH 残留编辑器；M347 Mapping DSL UI；M346 qualityChecks；M345 一元 `!`；M344 fieldKey
发现；M342 嵌套条件组；M341 EVIDENCE requiredWhen；M340 FORM ConditionBuilder；M339 UPDATE/CANCEL
Mapping；M336 RouteHint；M335 CREATE 强制 Mapping；M334 DSL；M333 无 fallback；M332 TECHNICIAN；
M331 出站仅 Mapping；M327～M321 已交付 PRICING/通知/RULE/DISPATCH/ASSIGNEE 与 Mapping。M267～M320
已交付多 OEM SPI、出站 Pipeline、远端查询、UNKNOWN/批量重放、六类 Runtime、设计器与三 OEM 冒烟。
Track A、独立 Network/Technician Web、iOS 与在线 Visit/表单/Evidence/Snapshot/整改仍有效。本机没有
Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、弱网/后台/
离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。

本地可无阻塞二次接线主线已收口于 M349。下一为硬门禁：吉利真实 Sandbox/凭据到位后优先联调；
AMOUNT/加权（待业务确认）与 BUSINESS 日历 SLA。Track F/G 仍为 `BLOCKED_EXTERNAL`。
OCR/CV、计算字段/脚本、BUSINESS 日历与 SLA 暂停/预警/升级、真实短信/邮件供应商
