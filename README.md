...
当前工程基线推进至 **M267**：在 M266 Technician 在线整改闭环之上，已交付多车企平台内核程序验收标准、
通用 Connector SPI 与 BYD CREATE_WORK_ORDER 入站管道归位，并以架构门禁阻断核心域依赖 OEM 适配包。
Track A、独立 Network/Technician Web、iOS Simulator/分发基础与在线 Visit/表单/Evidence/Snapshot/整改仍有效。
本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、弱网/后台/
离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。下一主线为配置治理 MVP、条件网关与
REFERENCE_OEM（见 `roadmap/06`）。
