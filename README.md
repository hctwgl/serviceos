...
- 独立 `readmodel` 模块可靠消费 WorkOrder/Workflow/Stage/Task、Appointment/Visit/ContactAttempt
  与 SLA 事件形成授权工单执行、现场履约与 SLA 时间线；
- Spring Modulith 边界验证与 PostgreSQL Testcontainers 集成测试。

当前工程基线推进至 **M75**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M75 073 75
```
...
[M75 工单 SLA 时间线事件合并](serviceos-architecture/architecture/88-m75-work-order-sla-timeline.md)。
