...
当前工程基线推进至 **M271**：在 M267 通用 Connector SPI / BYD 入站归位之上，已交付 Workflow
...
尚未验收。下一主线为 REFERENCE_OEM 与双车企回归（见 `roadmap/06`）。
