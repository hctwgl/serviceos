...
当前工程基线推进至 **M78**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M78 075 77
```
...
[M78 工单外发交付确认与恢复时间线事件合并](serviceos-architecture/architecture/91-m78-work-order-delivery-ack-timeline.md)。
