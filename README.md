...
当前工程基线推进至 **M280**：在 M267～M279 多 OEM 内核与复杂流程运行时之上，已交付工单取消级联、
授权重开、人工跳转，以及取消时按节点 `compensation` 声明创建补偿任务（Flyway V109）。Track A、
独立 Network/Technician Web、iOS Simulator/分发基础与在线 Visit/表单/Evidence/Snapshot/整改仍有效。
本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、
弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。下一主线为标准模板扩展
与领域配置设计器。
