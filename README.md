...
当前工程基线推进至 **M77**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M77 075 77
```
...
[M77 工单外发交付与异常闭环时间线事件合并](serviceos-architecture/architecture/90-m77-work-order-delivery-exception-timeline.md)。
