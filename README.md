...
- 独立 `readmodel` 模块可靠消费 WorkOrder/Workflow/Stage/Task 与 Appointment/Visit/ContactAttempt
  事件形成授权工单执行与现场履约时间线；
- Spring Modulith 边界验证与 PostgreSQL Testcontainers 集成测试。

当前工程基线推进至 **M74**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M74 072 74
```
...
[M74 工单现场履约时间线事件合并](serviceos-architecture/architecture/87-m74-work-order-field-ops-timeline.md)。
