...
当前工程基线推进至 **M71**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M71 070 72
```
...
[M71 Task 服务端允许动作投影](serviceos-architecture/architecture/84-m71-task-server-driven-allowed-actions.md)。
