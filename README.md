...
- `shared`、`bootstrap`、`identity`、`authorization`、`project`、`audit`、`reliability` 七个首切片模块；
...
这只是 M8/M9 的工程参考切片，不代表 M6 全部 E1 门禁、正式 IdP/Broker 或业务履约链路已经实现。
...
详细工程说明见 [M8 首条事务切片](serviceos-architecture/architecture/22-engineering-reference-implementation.md)和 [M9 身份授权与可靠消息](serviceos-architecture/architecture/23-identity-authorization-reliable-worker-implementation.md)。
