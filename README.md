...
当前工程基线推进至 **M86**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M86 078 80
```
...
[M86 工单时间线投影运行时硬化](serviceos-architecture/architecture/99-m86-work-order-timeline-projection-runtime-hardening.md)。
