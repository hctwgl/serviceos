...
当前工程基线推进至 **M296**：在 M267～M280 多 OEM 内核与复杂流程运行时之上，已交付工单取消级联、
授权重开、人工跳转、取消补偿，家充勘安/维修/移机/巡检标准模板，WORKFLOW/FORM/EVIDENCE/SLA 配置设计器 API，Admin 配置设计器壳，Diff/审批门禁，Bundle CANARY/回滚，Workflow 拖拽画布，CANARY 百分比流量灰度，画布建边/网关条件编辑，多槽位 CANARY 自动晋级，WORKFLOW 配置依赖分析，WORKFLOW 干跑模拟，Bundle 通道显式停用，十大配置资产类型设计器，以及冻结 Bundle 历史回放。Track A、独立
...
弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。下一主线为完整通用
Connector SPI（出站/回调/ACK）、配置资产运行时引擎与业务低代码设计器；Track F/G 与真实 OEM
Sandbox/凭据仍为 `BLOCKED_EXTERNAL` 并如实登记，不阻塞通用能力推进。
OCR/CV、计算字段/脚本、BUSINESS 日历与 SLA 暂停/预警/升级、通知运行时、履约事实与试算、对账结算、正式
IdP/Broker/对象存储/扫描服务、Consumer Identity、离线工作包、Outbound/回调全面 SPI 化以及其余 Network 写命令仍未实现。
