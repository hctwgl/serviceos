...
当前工程基线推进至 **M83**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M83 076 78
```
...
[M83 工单条件处置时间线事件合并](serviceos-architecture/architecture/96-m83-work-order-condition-disposition-timeline.md)。
