...
当前工程基线推进至 **M62**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M62 062 64
```
...
[M62 SLA 授权查询与工作台投影](serviceos-architecture/architecture/75-m62-sla-authorized-query-projection.md)。
