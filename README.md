...
- Admin Web 已进入 CI 阻断构建；开发态 Keycloak Authorization Code + PKCE 的真实
  ...
  check-in/check-out，以及 BYD 提审外发 ACK、厂端回调与入站→激活→同单预约上门的局部写链路。

当前工程基线推进至 **M140**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
[M140 Admin 入站激活与同单预约上门 E2E](serviceos-architecture/architecture/153-m140-admin-inbound-activate-field-ops-e2e.md)。
