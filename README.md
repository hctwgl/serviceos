...
当前工程基线推进至 **M334 入站 Mapping defaults/enum/condition DSL**（https://github.com/hctwgl/serviceos/pull/161；栈 `#148→…→#161`）
PREVIEW/SUBMIT/审批。M333 入站无适配器 fallback；M332 DISPATCH TECHNICIAN；M331 出站仅 Mapping；
M327～M321 已交付 PRICING/通知/RULE/DISPATCH/ASSIGNEE 与 Mapping。M267～M320 已交付多 OEM SPI、
出站 Pipeline、远端查询、UNKNOWN/批量重放后端、六类 Runtime、设计器与三 OEM 并行建单冒烟。
Track A、独立 Network/Technician Web、iOS 与在线 Visit/表单/Evidence/Snapshot/整改仍有效。
本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、
弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。

下一主线：吉利真实 Sandbox/凭据到位后优先联调；否则继续强制 INBOUND Mapping / DISPATCH 地图与低代码深化。
