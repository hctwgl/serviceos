...
当前工程基线推进至 **M70**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M70 070 72
```
...
[M70 授权任务队列与详情](serviceos-architecture/architecture/83-m70-authorized-task-directory-detail.md)。
