...
当前工程基线推进至 **M321**：在 M320 之上，冻结 INBOUND INTEGRATION Mapping 已物化为建单 Canonical
与领域命令（命中字段权威、`mappingVersionId`=`assetVersionId`、Canonical 嵌入 contentDigest）。
M267～M320 已交付多 OEM SPI、出站 Pipeline、远端查询、UNKNOWN/批量重放、六类 Runtime、设计器与三 OEM
并行建单冒烟。Track A、独立 Network/Technician Web、iOS Simulator/分发基础与在线 Visit/表单/Evidence/
Snapshot/整改仍有效。本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实
operationRef 签退、弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。

下一主线继续 **Configuration-Driven Fulfillment Runtime**：出站 Mapping、TaskAssignment/ServiceAssignment
自动接入、RULE 门禁、通知投递闭环与履约试算快照；吉利真实 Sandbox/凭据与 Track F/G 仍为
`BLOCKED_EXTERNAL`，不阻塞本地主线。OCR/CV、计算字段/脚本、BUSINESS 日历与 SLA 暂停/预警/升级、
真实短信/邮件供应商 Adapter、对账结算、正式 IdP/Broker/对象存储/扫描服务、Consumer Identity、
离线工作包以及其余 Network 写命令仍未实现。
