...
当前工程基线推进至 **M322**：在 M321 之上，冻结 OUTBOUND INTEGRATION Mapping 可生成提审 OEM Payload
（零 Mapping 仍兼容 Profile；`mappingVersionId`=`assetVersionId`）。M321 已将入站 Mapping 物化为建单
命令。M267～M320 已交付多 OEM SPI、出站 Pipeline、远端查询、UNKNOWN/批量重放、六类 Runtime、设计器与
...
下一主线继续 **Configuration-Driven Fulfillment Runtime**：TaskAssignment/ServiceAssignment 自动接入、
RULE 门禁、通知投递闭环与履约试算快照；吉利真实 Sandbox/凭据与 Track F/G 仍为 `BLOCKED_EXTERNAL`，
