...
当前工程基线推进至 **M297**：在 M267～M296 多 OEM 内核、复杂流程运行时与配置设计器之上，已交付
出站提审通用 Connector SPI 与 BYD 归位（技术 ACK 与业务 ACK 分离）。Track A、独立 Network/Technician
Web、iOS Simulator/分发基础与在线 Visit/表单/Evidence/Snapshot/整改仍有效。
本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、
弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。下一主线为回调/扩展
Connector SPI、配置资产运行时引擎与业务低代码设计器；Track F/G 与真实 OEM Sandbox/凭据仍为
`BLOCKED_EXTERNAL` 并如实登记，不阻塞通用能力推进。
OCR/CV、计算字段/脚本、BUSINESS 日历与 SLA 暂停/预警/升级、通知运行时、履约事实与试算、对账结算、正式
IdP/Broker/对象存储/扫描服务、Consumer Identity、离线工作包、回调全面 SPI 化以及其余 Network 写命令仍未实现。
