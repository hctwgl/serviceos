...
当前工程基线推进至 **M279**：在 M267～M278 多 OEM 内核与复杂流程运行时之上，已交付工单取消级联、
授权重开新建 ROOT 流程，以及人工跳转到定义内任务节点；事件 `workorder.cancelled` /
`workorder.reopened` 与 Flyway V106～V108 已落地。Track A、独立 Network/Technician Web、iOS
Simulator/分发基础与在线 Visit/表单/Evidence/Snapshot/整改仍有效。本机没有 Apple 签名身份或物理设备；
联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、弱网/后台/离线闭环、签名真机、真实 IdP、
VoiceOver 与 TestFlight 尚未验收。下一主线为异常补偿、标准模板扩展与领域配置设计器。
