...
- 20 个 Spring Modulith 模块，覆盖身份授权、配置、工单、工作流、任务、派单、SLA、预约、现场 Visit、
...
当前工程基线推进至 **M69**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M69 069 71
```
...
[M69 授权工单执行工作区投影](serviceos-architecture/architecture/82-m69-authorized-work-order-execution-projection.md)。
