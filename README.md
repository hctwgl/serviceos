...
当前工程基线推进至 **M81**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M81 076 78
```
...
[M81 工单 Task 指派与执行保护时间线事件合并](serviceos-architecture/architecture/94-m81-work-order-task-assignment-guard-timeline.md)。
