...
当前工程基线推进至 **M307**：在 M267～M306 之上，已交付出站提审/入站审核回调 SPI、Update/Cancel 入站 SPI，
以及 INTEGRATION/ASSIGNEE/DISPATCH/NOTIFICATION 冻结 Bundle 运行时。Track A、独立 Network/Technician Web、
iOS Simulator/分发基础与在线 Visit/表单/Evidence/Snapshot/整改仍有效。
本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、
弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。下一主线为 RULE/PRICING
运行时与业务低代码设计器；Track F/G 与真实 OEM Sandbox/凭据仍为 `BLOCKED_EXTERNAL` 并如实登记，
不阻塞通用能力推进。
