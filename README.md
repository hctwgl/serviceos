...
当前工程基线推进至 **M327**：`workorder.fulfilled` 提取最小履约事实并持久化 PRICING SHADOW
CalculationSnapshot（不落账）。M326 已交付 NOTIFICATION Intent/Delivery/Attempt；M325～M323 已交付
RULE/DISPATCH/ASSIGNEE 主链路接入；M321/M322 已交付入站/出站 Mapping。M267～M320 已交付多 OEM SPI、
出站 Pipeline、远端查询、UNKNOWN/批量重放、六类 Runtime、设计器与三 OEM 并行建单冒烟。Track A、
独立 Network/Technician Web、iOS 与在线 Visit/表单/Evidence/Snapshot/整改仍有效。本机没有 Apple
签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、弱网/后台/离线闭环、
签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。

下一主线继续 **Configuration-Driven Fulfillment Runtime**：UNKNOWN/Replay Admin；
吉利真实 Sandbox/凭据与 Track F/G 仍为 `BLOCKED_EXTERNAL`，不阻塞本地主线。OCR/CV、计算字段/脚本、BUSINESS 日历与 SLA 暂停/预警/升级、真实短信/邮件供应商
