...
当前工程基线推进至 **M88**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M88 078 80
```
...
[M88 工单工作区预约与到访区块](serviceos-architecture/architecture/101-m88-work-order-workspace-appointments-visits.md)。
