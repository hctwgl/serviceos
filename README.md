...
当前工程基线推进至 **M319**：在 M267～M306 之上，已交付出站提审/入站审核回调 SPI、Update/Cancel 入站 SPI，
以及 INTEGRATION/ASSIGNEE/DISPATCH/NOTIFICATION/RULE/PRICING 冻结 Bundle 运行时。Track A、独立 Network/Technician Web、
iOS Simulator/分发基础与在线 Visit/表单/Evidence/Snapshot/整改仍有效。
本机没有 Apple 签名身份或物理设备；联系人/预约写、完整高级表单与草稿、真实 operationRef 签退、
弱网/后台/离线闭环、签名真机、真实 IdP、VoiceOver 与 TestFlight 尚未验收。下一主线为 P3 设计器深化与第二 OEM（P4；缺 Sandbox 则 BLOCKED_EXTERNAL）；Track F/G 与真实 OEM Sandbox/凭据仍为 `BLOCKED_EXTERNAL` 并如实登记，
不阻塞通用能力推进。
OCR/CV、计算字段/脚本、BUSINESS 日历与 SLA 暂停/预警/升级、真实短信/邮件通知 Adapter、履约事实与试算、对账结算、正式
IdP/Broker/对象存储/扫描服务、Consumer Identity、离线工作包、远端查询 SPI 以及其余 Network 写命令仍未实现。
