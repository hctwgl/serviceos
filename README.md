...
当前工程基线推进至 **M98**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M98 082 84
```
...
[M98 授权整改案例队列](serviceos-architecture/architecture/111-m98-authorized-correction-case-queue.md)。
