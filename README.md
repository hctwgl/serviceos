...
这只是 M8～M25 的工程参考切片。M25 接通师傅改派的 Dispatch/Task Inbox saga，但初派、Network 双级派单和自动补偿仍未闭环；不代表 M6 Foundation 全部 P0、正式 IdP/Broker/对象存储/反病毒服务、策略解析、SLA、网关/并行或完整业务履约链路已经实现。
