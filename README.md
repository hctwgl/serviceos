...
当前工程基线推进至 **M272**：在 M267 通用 Connector SPI / BYD 入站归位之上，已交付 Workflow
...
尚未验收。下一主线为双车企端到端回归与第三家接入手册（见 `roadmap/06`）。
