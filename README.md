...
当前工程基线推进至 **M99**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M99 083 85
```
...
[M99 授权外发交付队列](serviceos-architecture/architecture/112-m99-authorized-outbound-delivery-queue.md)。
