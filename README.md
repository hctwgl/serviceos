...
当前工程基线推进至 **M64**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M64 064 66
```
...
[M64 项目区域关系与 REGION SLA 队列](serviceos-architecture/architecture/77-m64-project-region-scope-sla-queue.md)。
