...
当前工程基线推进至 **M340 FORM ConditionBuilder 条件积木嵌入**（https://github.com/hctwgl/serviceos/pull/167；栈 `#148→…→#167`）。
M339 UPDATE/CANCEL Mapping；M338 比例缺口；M337 Coverage；M336 RouteHint；
M335 CREATE 强制 Mapping；M334 DSL；M333 无 fallback；M332 TECHNICIAN；M331 出站仅 Mapping；
M327～M321 已交付 PRICING/通知/RULE/DISPATCH/ASSIGNEE 与 Mapping。
...
下一主线：吉利真实 Sandbox/凭据到位后优先联调；否则继续嵌套表达式/EVIDENCE 积木等低代码深化。
