...
当前工程基线推进至 **M80**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M80 076 78
```
...
[M80 工单服务分配生命周期时间线事件合并](serviceos-architecture/architecture/93-m80-work-order-assignment-timeline.md)。
