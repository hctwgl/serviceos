...
当前工程基线推进至 **M68**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M68 068 70
```
...
[M68 授权工单目录与详情查询](serviceos-architecture/architecture/81-m68-authorized-work-order-query.md)。
