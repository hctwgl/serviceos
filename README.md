...
当前工程基线推进至 **M84**，但仍是模块化后端纵向切片，不代表完整现场履约平台已经交付。
...
```bash
bash scripts/check-milestone-consistency.sh M84 077 79
```
...
[M84 工单时间线投影 Checkpoint 与重建](serviceos-architecture/architecture/97-m84-work-order-timeline-projection-checkpoint-rebuild.md)。
